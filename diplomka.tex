...
For a configuration $c$, we define $\mathit{cover}(c) = \{c_1, \dots, c_{|R|}\}$ to be the \emph{cover} of $c$, representing all the nodes currently visited by at least one robot.
A cover can contain $1$ to $|R|$ members.
From now on, when we say that a node is \emph{visited} (by the defender or by a configuration), we mean that at least one of the robots is located in the node.
%For a node $u \in U$, we define $C_u = \{c \in C \mid u \in \mathit{cover}(c)\}$ to be the set of all configurations that visit $u$.
...
The definition of the attacker's strategy is the~same as in the SRPG---the attacker can still attack only a single node and only once during a play.
...
In this section, we show that there exists an optimal defender's strategy for each MRGP.
We follow the steps from Section \ref{sec:translation} where we did the same for SRGPs.
We redefine a $k$-commitment to reflect the changes in the definition of the defender's strategy and we briefly describe the process of translating an MRPG to a stochastic game and vice versa.
As stated in \cite{iti1}, all the needed modifications for more general models (in our case, the multi-robot model) are straightforward and if the reader is familiar with the translation process for SRPGs, we advise him or her to skip this section.
...
We also adopt the concept of extending a $d$-commitment and now we are prepared for the translation of the patrolling game $\mathcal{G}$ to a stochastic game.
We define a stochastic game $\mathcal{S} = (V, (V_{\emph{Min}}, V_{\emph{Max}}),$ $|C|^{d+2}, A, \mathbf{E}, \mathbf{P})$ in the same way as we did in Section \ref{sec:translation} (taking into account the modified definition of a $d$-commitment) with the only exception that $\mathbf{E}(\mathit{init}) = \mathbf{Comm}_d$, as the initial configuration is strategy-specific and therefore cannot be forced.
