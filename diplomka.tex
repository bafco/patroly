...
And for the automaton's intial state it suffices to choose some $\hat{c} \in B$ such that $\hat{c}(0, \hat{u}) = 1$.\footnote{There has to be at least one such $\hat{c} \in B$ unless the value of all characteristics from $B$ is 0.}
...
As we often refer to some parts from those chapters, it is a huge advantage for the reader to be familiar with them.
...
We see that the behavior of the strategy in the first two steps (for histories of length $0$ and $1$) is encoded by the initial state $q_0$.
...
Therefore, we provide a detailed formal argumentation about the algorithm.
...
Thus, the $c$-successor of a charateristic $(f, m)$ can be an arbitrary characteristic as long as $\xi(f)(c) = 0$.
\end{remark}
...
\end{align}
The equality \eqref{eq6:second} follows from the induction hypothesis (we used notation $\mathbf{Char}_{\gamma_i} = (f_{\gamma_i}, m_{\gamma_i})$) and the fact that $f = f_\gamma$.
...
% - mnozinu vrcholov (namiesto |U|^z by ich bolo "len" pocet \-kombinacii z (|U|+z-1)-prvkovej mnoziny)
% - mnozinu stavov
%
%TODO spell checker
