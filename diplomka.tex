...
The algorithm is based on searching for a \emph{closed} set of such commitments (the defender stays within this set by prolonging her plans) that maximizes the minimal value of all commitments.
...
To reduce the number of possible states of an automaton encoding the $\epsilon$-optimal strategy, we introduce the concept of some specific \emph{characteristics} of a state.
These characteristics represent less information than a $d$-commitment.
However, as we will see, they are enough to find the desired strategy.

%TODO treba mat automat \mathcal{A} zo Step 3.

%IDEA zamenit znacenie: 'f' namiesto 'p' ??? Radsej nie: f, g, h -> nemozem pouzit 'h' ako historiu
\begin{definition}
...
%TODO premiestit do dokazu?
For every state $p \in Q$, we define the value of its characteristics by $val(\mathbf{Char}_p) = \min_{t \in T} \mathbf{Char}_p(d, t)$.
Note that if $p = \mathcal{A}[h]$ for some history $h$, %and $\iota$ is the strategy determined by $\mathcal{A}$
then $val(\mathbf{Char}_p) = att\text{-}val(\sigma_\epsilon^{reg} \mid h)$.
...
$$\mathbf{Char}_r^{\mathcal{A}_{p \leftarrow q}} = \mathbf{Char}_r^\mathcal{A}~.$$
\end{lemma}
...
$\mathbf{Char}_p(1, u) = \xi(p)(u)$.
for $2 \leq k \leq d$ we have that $\mathbf{Char}_p(k, u) = \xi(p)(u) + \sum_{v \neq u}\xi(p)(v) \cdot \mathbf{Char}_{\delta(p, v)}(k - 1, u)$.
\end{proof}
