...
Therefore, every player $P_i$ has his own \emph{utility function} $U_i: \mathcal{S} \mapsto \mathbb{R}$ which assigns \emph{payoffs} to strategy profiles.
...
When it comes to solving the game, some form of equilibrium strategy profile is in demand. The best known solution concept in game theory is \emph{Nash equilibrium} (NE), in which no player can gain anything by changing only his strategy. That is, each player's strategy is a best response to the other players' strategies.
...
Security games are two-player games in which \emph{defender} allocates resources to guard targets against an \emph{attacker} (also \emph{intruder} or \emph{adversary}), who aims to attack some target.
...
The standard solution concept for Stackelberg games is \emph{strong Stackelberg equilibrium} (SSE, also \emph{leader-follower equilibrium}), which gives the leader the maximum expected utility in the worst-case scenario.
...
One of the advantages of an SSE is that being a Stackelberg leader cannot hurt the defender \cite{convex}. Furthermore, under certain natural assumptions about security games, any SSE is also an NE \cite{interchangability}.
...
Another example of a game-theoretic security system is \emph{PROTECT} (Port Resilience Operational/Tactical Enforcement to Combat Terrorism) \cite{protect}.
...
In contrast to the three beforementioned security applications, PROTECT relies on a quantal responce (QR) \cite{quantal} model of the adversary's behavior, i.e. attacker has only bounded rationality.
...
The main purpose of TRUSTS is fare-evasion deterrence in urban proof-of-payment transit systems, where passengers are legally, but not physically, required to purchase tickets.
...
In this section, we give an overview of the research in the field of the patrolling games in the last years. We address several models. For each model, we state the differences from our model and we mention the most significant results.

In the rest of the section, we use the following assumptions which hold true unless stated otherwise.

Patrolling games are played on directed graphs with distance between any two neighbour nodes being the same.
The terms \emph{patroller} and \emph{robot} refer to the defender.
The terms \emph{intruder} and \emph{adversary} refer to the attacker.
We assume single adversary.
...
We assume strong adversarial setting, i.e. the attacker knows the defender's strategy and the current position of patrolling unit(s).

attack lenght = penetration time
...
\bibitem{convex} B. von Stengel and S. Zamir. Leadership Games with Convex Strategy Sets. \emph{Games and Economic Behavior}, volume 69, pages 446ñ457, 2010.

\bibitem{interchangability} Z. Yin, D. Korzhyk, C. Kiekintveld, V. Conitzer, and M. Tambe. Stackelberg vs. Nash in Security Games: Interchangeability, Equivalence, and Uniqueness. In \emph{AAMAS}, pages 1139ñ1146, 2010.

\bibitem{limited1} J. Pita, M. Jain, M. Tambe, F. OrdÛ$\rm{\tilde{n}}$ez, and S. Kraus. Robust Solutions to Stackelberg Games: Addressing Bounded Rationality and Limited Observations in Human Cognition. \emph{Artificial Intelligence}, volume 174, pages 1142-1171, 2010.

\bibitem{limited2} B. An, D. Kempe, C. Kiekintveld, E. Shieh, S. Singh, M. Tambe, and Y. Vorobeychik. Security Games with Limited Surveillance. In \emph{AAAI}, pages 1241ñ1248, 2012.

\bibitem{limited3} B. An, M. Brown, Y. Vorobeychik, and M. Tambe. Security Games with Surveillance Cost and Optimal Timing of Attack Execution. In \emph{AAMAS}, pages 223ñ230, 2013.

\bibitem{limited4} Y. Zhang and X. Luo. Security Games with Partial Surveillance. In \emph{AAMAS}, pages 1527-1528, 2014.

\bibitem{quantal} R. D. McKelvey and T. R. Palfrey. Quantal Response Equilibria for Normal Form Games. \emph{Games and Economic Behavior}, volume 10, pages 6ñ38, 1995.

\bibitem{armor} J. Pita, M. Jain, J. Marecki, F. OrdÛ$\rm{\tilde{n}}$ez, C. Portway, M. Tambe, C. Western, P. Paruchuri, and S. Kraus. Deployed ARMOR Protection: The Application of a Game Theoretic Model for Security at the Los Angeles International Airport. In \emph{AAMAS}, pages 125ñ132, 2008.

\bibitem{iris} J. Tsai, S. Rathi, C. Kiekintveld, F. OrdÛ$\rm{\tilde{n}}$ez, and M. Tambe. IRIS - A Tool for Strategic Security Allocation in Transportation Networks. In \emph{AAMAS}, pages 37-44, 2009.
