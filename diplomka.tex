...
\textbf{Step 3.}
%TODO %IDEA $q_0$ spravit ako "nelegalny" commitment
...
\begin{definition}
Given $p \in Q$, we define the characteristics $\mathbf{Char}_p$ of $p$ to be a function which to every pair $(k, u) \in \{0, \dots, d\} \times U$ assigns the probability of reaching $u$ in at most $k$ steps whenever the strategy is in the state $p$ (recall that $p$ determines the current state of the game).
...
%TODO - zmazat? - toto znacenie nikde nepotrebujem - len v Lemma-tku + v dokaze uz pouzivam $\iota_{nieco}$
%We denote by $\sigma_{p \leftarrow q}$ the strategy determined by $\mathcal{A}_{p \leftarrow q}$.
...
where $\mathbf{Char}_r^{\mathcal{A}_{p \leftarrow q}}$ denotes the characteristics of $r$ in the automaton $\mathcal{A}_{p \leftarrow q}$.
\end{lemma}

\begin{proof}
% automat je Markov Chain!
...
This brings us to the main idea of the proof, particularly that the characteristics of a state $r$ are determined by the distribution in its root, by the transitions from it, and by the characteristics of all states to which there is a transition from $r$ under some node $u$.

%Characteristics can be inductively defined as follows: (based on their semantics)
$\mathbf{Char}_r(1, v) = \xi(r)(v)$.

for $2 \leq k \leq d$ we have that
