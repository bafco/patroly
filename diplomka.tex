...
We provide a quadratically exponential algorithm which computes an $\epsilon$-optimal defender's strategy for a given patrolling game and a given precision $\epsilon$.
Further, we extend the model of patrolling games by allowing multiple patrolling robots.
We provide an algorithm to compute an $\epsilon$-optimal defender's strategy in doubly exponential time in the number of robots.
...
Patrolling games are security-oriented games played between the \emph{defender} and the \emph{attacker}, mostly on an oriented graph.
...
During this time, he can be caught by the defender.
...
Some of the nodes are targets, which represent some value and are subject to attacks.
...
There is one virtue specific to our model which is not covered by any other papers prior to \cite{miso} and that is the infinite horizon.
...
In Chapter \ref{chap:preliminaries}, we present the basics of game theory and we introduce the security games and patrolling games in general.
...
We provide a doubly exponential algorithm to compute an $\epsilon$-optimal defender's strategy.
...
Authors' subsequent papers concerning multi-robot perimeter patrol in adversarial settings include \cite{perimeter2} and \cite{perimeter3}. In \cite{perimeter2}, it is shown that in the case the adversary has no knowledge of the patrol scheme, a deterministic algorithm is optimal. However, the deterministic algorithm creates high deviations between the  \textsf{ppd} values in the nodes and it is easy to detect. Therefore, the authors present an algorithm that deals with adversary having partial information. This algorithm maximizes the expected \textsf{ppd} along with minimizing the deviation between the \textsf{ppd} values throughout the perimeter. Its performance was verified by experiments with human subjects.
...
and a linear program to find the smallest labelled clique cover. The result of the program is the smallest number of robots needed to patrol the environment.
...
In this section, we provide an algorithm to compute a regular $\epsilon$-optimal strategy of a given MRPG for a given precision $\epsilon$.
...
The purpose of this thesis was to extend the previous results for patrolling games (provide a more efficient algorithm to solve instances of such games) as well as to define a more general model and apply the achieved results to it.
...
We provided an algorithm for computing an $\epsilon$-optimal strategy in only quadratically exponential time.
...
We adapted the results for single-robot patrolling games to multi-robot patrolling games and provided an algorithm to compute an $\epsilon$-optimal strategy for any MRPG.
