...
\begin{definition}
Given $p \in Q$, we define the characteristic $\mathbf{Char}_p$ of $p$ to be a function which to every pair $(k, u) \in \{0, \dots, d\} \times U$ assigns the probability of reaching $u$ in at most $k$ steps whenever the strategy is in the state $p$ (recall that $p$ determines the current state of the play).
\end{definition}
...
%Note that the characteristic of a state preserves the information about probability distribution in its root.
