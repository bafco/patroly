...
%At many places we consider strategies obtained by “forgetting” some initial prefix of the history.
%Formally, for all $h \in \mathcal{H}$ and $\theta \in \Sigma \cup \Pi$, we define a strategy $\theta_h$ by $\theta_h(last(h)h') = \theta(hh')$ for all $h' \in U*$.
...
We discretize $\sigma$ to obtain a strategy $\sigma_\epsilon$ with all probabilities $\sigma_\epsilon(h)(u)$ being integer multiples of an appropriate number $c$ satisfying \mbox{$c = 1/l$} for some $l \in \mathbb{N}$.
%TODO
...
We can guarantee this property for $c = \lceil(|N|d)/\epsilon\rceil^{-1}$ using the following rounding procedure.

Let $U = \{u_1, u_2, \dots, u_{|U|}\}$ be the set of nodes of $\mathcal{G}$.
For every history $h$ and every $1 \leq i \leq |U|$, we inductively define $\sigma_\epsilon(h)(u_i) = k_i \cdot c$, where $k_i$  is the largest number satisfying
$$ k_i \cdot c \leq \sigma(h)(u_i) + \sum_{j=1}^{i-1}(\sigma(h)(u_j) - \sigma_\epsilon(h)(u_j))~.$$
...
%TODO preformulovat ?
Further, when we realize the invariant $0 \leq \sum_{j=1}^{i-1}(\sigma(h)(u_j) - \sigma_\epsilon(h)(u_j)) < c$ holds for all $1 \leq i \leq |U|$, it is easy to see that $|\sigma(h)(u_i) - \sigma_\epsilon(h)(u_i)| < c$, which is captured by the following lemma.% (for all $i$)
\begin{lemma}
\label{lem:difference}
For all $h \in \mathcal{H}$ and $u \in U$ we have $|\sigma(h)(u) - \sigma_\epsilon(h)(u)| < c$.
\end{lemma}
...
%TODO zmazat, vobec netreba!
%TODO presunut niekam prec - do apendixu?; je to vskutku "lemma"?
%In a later proof, we will need this ancillary lemma.
%%TODO absolutnu hodnotu mi nalavo netreba
%\begin{lemma}
%\label{lem:inequality}
%For all $a, a', b, b' \in [0, 1]$ we have
%$$|ab - a'b'| \leq |a - a'| + |b - b'|~.$$
%\end{lemma}
%
%\begin{proof}
...
%  &\leq b|a - a'| + a'|b - b'|\nonumber~.
%\end{align}
%%$|ab - a'b'| = |ab - a'b + a'b - a'b'| = |b(a - a') + a'(b - b')| \leq b|a - a'| + a'|b - b'|$
%\end{proof}
...
\begin{align}
R[\sigma, h, t, k] - R[\sigma_\epsilon, h, t, k] \leq k|U|c~.\nonumber %\label{eq1:induction}
\end{align}
...
\Delta
  ...
  &\leq& \sum_{u\in U} R[\sigma, hu, t, k - 1]\cdot c + \sum_{u\in U} \sigma_\epsilon(h)(u)\cdot (k - 1)|U|c	\label{eq1b:third} \\
	%  &\leq& \sum_{u\in U} R[\sigma, hu, t, k - 1]\cdot c +	\label{eq1b:third} \\
	%  &\hspace{5pt}+\hspace{-5pt}& \sum_{u\in U} \sigma_\epsilon(h)(u)\cdot (k - 1)|U|c \nonumber \\
  &\leq& |U|c + (k - 1)|U|c	\label{eq1b:fourth} \\
  &=& k|U|c\nonumber~.
%\end{align}
\end{eqnarray}
\begin{itemize}[noitemsep]
...
\end{itemize}
So we have that $R[\sigma, h, t, d] - R[\sigma_\epsilon, h, t, d] \leq d|U|c$.
However, note that
$$att\text{-}val(\iota \mid h) = \inf_{t \in T} R[\iota, h, t, d]$$
and therefore
$$att\text{-}val(\sigma \mid h) - att\text{-}val(\sigma_\epsilon \mid h) \leq d|U|c \leq \epsilon~.$$
\end{itemize}
\end{proof}
...
%For $r \in Q \smallsetminus \{q\}$, we denote by $\mathbf{Char}'_r$ the characteristics of $r$ in the automaton $\mathcal{A}_{p \leftarrow q}$.
...
Let us denote by $\mathcal{A}_{red}$ %= (Q_{red} \cup \{q_0\}, U, \delta_{red}, q_0, \{\})$
the resulting automaton which cannot be further reduced by applying Lemma \ref{lem:characteristics}.\footnote{BLABLABLA non-deterministic} %TODO
...
%By comparing the automata $\mathcal{A}$ and $\mathcal{A}_{red}$, we will now prove that $\sigma_{red}$ is $\epsilon$-optimal.
...
\begin{lemma}
For each state $p \in Q_{red}$ we have $val(\mathbf{Char}_p) \geq val(\sigma) - \epsilon$.
\end{lemma}
\begin{proof}
...
Therefore, we have that $val(\mathbf{Char}_q) = val(q)$ for each $d$-commitment $q \in Q$.
Further, from Lemma \ref{lem:epsilon_commitment} we have that $val(q) \geq val(\sigma) - \epsilon$ for each $q \in Q$.
However, $Q_{red} \subseteq Q$ and the characteristics of $p$ are the same in both $\mathcal{A}$ and $\mathcal{A}_{red}$ based on Lemma \ref{lem:characteristics}.
\end{proof}
