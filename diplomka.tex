...
%TODO BETTER
%\begin{proof}
%TODO jednoduchy dokaz Lemma value (5.1.2) - vezmem (pociatocny) vrchol, pre kt. ma hra (opt. strategia) najvacsiu hodnotu -> val(\sigma | h) potom nemoze byt vacsia pre ziadne h / vsetko, kam pridem, ma mensiu alebo rovnu hodnotu
%We provide much shorter proof as the one in \cite{iti1}.
%The main idea of our proof is that we consider a history $h_{max}$ after which the strategy $\sigma$ has the largest value.
%We show that after $h_{max}$ is played, the value of $\sigma$ cannot increase, nor decrease.
%From that, we can derive an optimal strategy whose value is the same (largest) after all histories.
%Formally, let us consider a history $h_{max} \in \mathcal{H}(\sigma)$ such that $val(\sigma \mid h_{max}) \geq val(\sigma \mid h)$ for all $h \in \mathcal{H}(\sigma)$.	% $... = \sup_{h \in \mathcal{H}(\sigma)} ...$
%TODO preco by taka mala existovat? co ak sa to stale zvacsuje a limitne blizi k nejakej hodnote?
%\end{proof}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$\delta(q_0, \hat{u}) = \theta_\emph{Max}^{ml}(\mathit{init})$ and
...
\end{itemize}
%TODO niekde sa zmienit, ze 'in the future, by \emph{states} we refer to $Q$'
\vspace{-3pt}
...
%TODO namiesto $Q_0'$ napisat v definicii automatu rovno $Q_0 \smallsetminus \{q\}$
%IDEA pripadne zadefinovat $Q'$
...
Let us fix and denote by $\mathcal{A}_{red}$ %= (Q_{red} \cup \{q_0\}, U, \delta_{red}, q_0, \{\})$
...
We denote by $Q_{red}$ the set of non-initial states of $\mathcal{A}_{red}$ (without $q_0$) and
by $\sigma_{red}$ the defender's strategy determined by $\mathcal{A}_{red}$.
...
\\ $Q = E$
\\ $q_0 = c$ such that $c(0, \hat{u}) = 1$ (there has to be such $c$ unless the value of all characteristics from $E$ is 0), resp. prechod z $q_0$ do $c$ pod $\hat{u}$
...
%centralne riadena strategia
%^^-> remark - diskusia k tomu, ze inak by nebola taka hodnota + future work sa moze ubrat tymto (inym) smerom + ze to nie je len trivialne rozsirenie 1 defendera
...
%TODO list
%TODO game -> strategic form
%TODO jednoduchy dokaz Lemma value (5.1.2) - vezmem (pociatocny) vrchol, pre kt. ma hra (opt. strategia) najvacsiu hodnotu -> val(\sigma | h) potom nemoze byt vacsia pre ziadne h / vsetko, kam pridem, ma mensiu alebo rovnu hodnotu
%TODO oddelat q_0 -> pouzit nieco ako $h^+$ - bez pociatocneho symbolu -> sigma(h) = A[h^+], kde h_0.h^+ = h
