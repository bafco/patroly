...
Given $p \in Q$, we define the characteristics $\mathbf{Char}_p$ of $p$ to be a function which to every pair $(k, u) \in \{1, \dots, d\} \times U$ assigns the probability of reaching $u$ in at most $k$ steps whenever the strategy is in the state $p$ (recall that $p$ determines the current state of the game).
...
This change in the transition function might influence the characteristics of all the states from which $q$ was reachable in less than $d$ steps.
%To be able to distinguish between the old and new characteristics of a state $r$,
For $r \in Q$, we denote by $\mathbf{Char}_r^{\mathcal{A}_{p \leftarrow q}}$ the characteristics of $r$ in the automaton $\mathcal{A}_{p \leftarrow q}$.
...
$$\mathbf{Char}_r^{\mathcal{A}_{p \leftarrow q}} = \mathbf{Char}_r~.$$
\end{lemma}

\begin{proof}
Our first observation is that the automaton $\mathcal{A}$ together with the strategy it determines can be seen as a Markov Chain.
%TODO premenovat $f$ a $g$ na $r$ a $s$
Indeed, when the game is in a state $f$, the next state $g$ is chosen with probability $\sum_{u \in U, g = \delta(f, u)} \xi(f)(u)$.
...
Notice that although the states of $\mathcal{A}$ are $d$-commitments, only the probability distribution in their root affects the strategy.
...
This brings us to the main idea of the proof, particularly that the characteristics of a state $r$ are completely determined by the probability distribution in its root, by the transitions from it, and by the characteristics of all the states to which there is a transition from $r$ under some node $u$.
%Characteristics can be inductively defined as follows: (based on their semantics)
%Note that the characteristics of a state preserve the information about probability distribution in its root.
Formally, let $v \in U$ be an arbitrary node.
We have that
$\mathbf{Char}_r(1, v) = \xi(r)(v)$
and for all $2 \leq k \leq d$ we have that
$\mathbf{Char}_r(k, v) = \xi(r)(v) + \sum_{u \neq v}\xi(r)(u) \cdot \mathbf{Char}_{\delta(r, u)}(k - 1, v)$.
This follows from the definition of the characteristics.
\end{proof}
