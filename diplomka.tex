...
\State \textbf{for each} $c \in \mathbf{Char}^s$ \textbf{do}
...
We construct a DFA $\mathcal{A}_{R} = (R, U, \delta_{R}, \hat{c}, \{\})$ where $\delta_{R}(c, u_i) = c_i$ for all $c \in R$ and $u_i \in U$.
%\vspace{-3pt}
%\begin{itemize}
...
It follows from Lemma \ref{lem:char=char} that $val(\sigma_{R}) \geq \min_{c \in R} val(c)$.
