...
Applying results for perfect-information stochastic games with finitely many states (see \cite{kucera}), we obtain an optimal \emph{memoryless} strategy $\theta_\emph{Max}^{ml}$ such that
...
\textbf{Step 3.} 
\\
$\sigma_\epsilon^{reg}(h) = f(r(f))$ where $f = \mathcal{A}[h]$ is the state of $\mathcal{A}$ reached after reading the history $h$.

tiez by to slo ako $\sigma_\epsilon^{reg}(h) = \mathcal{A}[h](last(h))$
