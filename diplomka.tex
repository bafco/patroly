...
\end{proof}
\noindent
We further proceed in the same way as in Section \ref{sec:characteristics}.
...
\begin{definition}
Given a state $p \in Q$, we define the value of its characteristic by $$val(\mathbf{Char}_p) = \min_{t \in T} m_p(d, t)~.$$
\end{definition}
Note that $att\text{-}val(\sigma_{red} \mid h) = val(\mathbf{Char}_p)$ where $p = \mathcal{A}[h^+]$.
