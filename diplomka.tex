...
In step 1, we give a more detailed proof of the $\epsilon$-optimality of the discretized strategy and we divide it into multiple lemmas. %which we will use later
...
\end{definition}
\noindent
Note that, for all $h \in \mathcal{H}$, $supp(\sigma_h(\epsilon)) \subseteq succ(last(h))$ holds by the definition of a defender's strategy and the fact that $\sigma_h(\epsilon) = \sigma(h)$.
