...
The base case ($k = 1$) directly follows from Lemma \ref{lem:difference} and the fact that $R[\iota, h, u, 1] = \iota(h)(u)$. %for every defender's strategy $\iota$.
...
%Using this, we straightforwardly modify the definitions from SRPGs to work with configurations instead of single nodes.
...
Now, as a defender's strategy needs to be defined also for an empty word, we modify the definition of a defender's strategy after a history has been played.

\begin{definition}
Let $\sigma \in \Sigma$ be a defender's strategy and $h \in \mathcal{H}$ a history. %TODO $h \in \mathcal{H}_0$ ???
We define a strategy $\sigma_h$ by $\sigma_h(h') = \sigma(hh')$ for all $h' \in \mathcal{H}_0$.
Further, we define the value of $\sigma$ after a history $h$ has been played by $val(\sigma \mid h) = val(\sigma_h)$.
\end{definition}
\noindent
Note that, for all $h \in \mathcal{H}$, we have that $supp(\sigma_h(\epsilon)) \subseteq succ(last(h))$ by the definition of a defender's strategy and the fact that $\sigma_h(\epsilon) = \sigma(h)$.
Also note that $\sigma_{\sigma(\epsilon)}(\epsilon) = \sigma(\sigma(\epsilon))$ is the probability distribution in the $\sigma$-initial configuration.
...
%TODO charakteristika - ako predtym, ale treba taktiez uchovat distribuciu v koreni, nakolko ta sa uz neda odvodit
