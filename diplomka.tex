...
\begin{proof}
As a state completely determines the $1$-commitment of its characteristic, we have $f_r = f'_r$ for every $r$.
...
\begin{algorithmic}[1]
\par\vspace{0.5mm}
	\Require	multi-robot patrolling game $\mathcal{G} = (U, E, T, d, R)$, $\epsilon > 0$
	\Ensure	a regular $\epsilon$-optimal strategy $\sigma_\epsilon^{alg}$
\par\vspace{3.5mm}

\State $A \leftarrow$ empty priority queue; $D \leftarrow \emptyset$
\State \textbf{for each} $a \in \mathit{Char}^s$ \textbf{do}
		add $a$ to $A$ with priority $val(a)$
\vspace{3px}
\Repeat
	\State pull from $M$ the item $a$ with the highest priority
	\State $D \leftarrow D \cup \{a\}$
\Until {$D$ is \emph{complete}}
\vspace{3px}
\State $\sigma_\epsilon^{alg} \leftarrow$ the strategy induced by a closed subset of $D$
\end{algorithmic}
