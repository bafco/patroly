...
Note that the values for $k = 1$ represent the probability distribution over the nodes in the next step as $\mathbf{Char}_{\delta(r, u)}(0, v) = 0$ for all $u \neq v$.
...
So far, we have shown that there is a DFA $\mathcal{A}_{red}$ encoding an $\epsilon$-optimal strategy.
Moreover, this automaton
%TODO uviest to tu
