...
Using this definition of successors, we straightforwardly modify the definitions of \emph{path}, \emph{history}, and \emph{run} from patrolling game to work with configurations instead of single nodes.
...
As there are three targets and only two robots, the targets cannot be defended better than with probability $2/3$, which is clearly the value of the game.
...
Every optimal strategy has to visit each target with probability $2/3$ in each step.
Note that it is impossible for a decentralized/disjointed strategy
...
%TODO hodilo by sa mi nieco ako $cover(c) = \{c_1, \dots, c_{|R|}\}$ pre configuration $c$
