...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\vspace{-2pt}
		\item $\mathbf{P}((\emph{Max}, f), g)((\emph{Min}, f')) = f(r(f))(u)$ where $f'$ is the $u$-suffix~of~$g$
...
\chapter{Main Result}
%TODO chapter intro
%TODO 2 odstavce: 1) co uz bolo dosiahnute; 2) v com som lepsi
...
Let us consider a game $\mathcal{G}_v$ with the largest value among all such games $\mathcal{G}_u$, i.e. $val_v = \sup_{u \in U} val_u$, and let $\sigma^v$ be an optimal defender's strategy in $\mathcal{G}_v$.
...
A defender’s strategy $\sigma$ is \emph{regular} if there is a deterministic finite-state automaton (DFA) $\mathcal{A}$ over the alphabet $U$ (the set of nodes) such that $\sigma(h)$ depends only on the control state entered by $\mathcal{A}$ after reading $h$ \cite{iti2}.
...
We can guarantee this property for $s = \lceil(|N|d)/\epsilon\rceil^{-1}$ using the following rounding procedure.
...
Further, when we realize the invariant $0 \leq \sum_{j=1}^{i-1}(\sigma(h)(u_j) - \sigma_\epsilon(h)(u_j)) < s$ holds for all $1 \leq i \leq |U|$, it is easy to see that $|\sigma(h)(u_i) - \sigma_\epsilon(h)(u_i)| < s$, which is captured by the following lemma. %TODO zmazat "which is captured..."
\begin{lemma}
...
%IDEA - presunut dokaz niekam inam?
\begin{proof}
...
We prove by induction on $k$ that for all $h \in \mathcal{H}$, $t \in T$, and $k \in \mathbb{N}_0$ we have that
\begin{align}
R[\sigma, h, t, k] - R[\sigma_\epsilon, h, t, k] \leq k|U|s~.\nonumber %\label{eq1:induction}
\end{align}
...
For $k \geq 1$, we have
\begin{eqnarray}
%TODO pridat zatvorku za sumu? viac odsadit?
\Delta
  &=& \sum_{u\in U}\sigma(h)(u)\cdot R[\sigma, hu, t, k - 1] - \label{eq1b:first} \\
  & &\hspace{7.6pt} -~\sigma_\epsilon(h)(u)\cdot R[\sigma_\epsilon, hu, t, k - 1]	\nonumber \\
  &=& \sum_{u\in U} R[\sigma, hu, t, k - 1]\cdot (\sigma(h)(u) - \sigma_\epsilon(h)(u)) \label{eq1b:second} +	\\
  & &\hspace{7.6pt} +~\sigma_\epsilon(h)(u)\cdot (R[\sigma, hu, t, k - 1] - R[\sigma_\epsilon, hu, t, k - 1]) \nonumber	 \\
  &\leq& \sum_{u\in U} R[\sigma, hu, t, k - 1]\cdot s + \sum_{u\in U} \sigma_\epsilon(h)(u)\cdot (k - 1)|U|s	\label{eq1b:third} \\
	%  &\leq& \sum_{u\in U} R[\sigma, hu, t, k - 1]\cdot s +	\label{eq1b:third} \\
	%  &\hspace{5pt}+\hspace{-5pt}& \sum_{u\in U} \sigma_\epsilon(h)(u)\cdot (k - 1)|U|s \nonumber \\
  &\leq& |U|s + (k - 1)|U|s	\label{eq1b:fourth} \\
  &=& k|U|s\nonumber~.
\end{eqnarray}
\begin{itemize}
	\item The equality \eqref{eq1b:first} follows from the definition of $R[\iota, h, t, k]$ as $R[\iota, h, t, k] = \sum_{u \in U} \iota(h)(u) \cdot R[\iota, hu, t, k - 1]$ for all $k \in \mathbb{N}$. %$k \geq 1$.
	\item The equality \eqref{eq1b:second} is just an application of the formula $ab - a'b' = b(a -  a') + a'(b - b')$.
	\item The inequality \eqref{eq1b:third} follows from Lemma \ref{lem:difference} and from the induction hypothesis.
	\item The inequality \eqref{eq1b:fourth} holds because $R[\sigma, hu, t, k - 1] \leq 1$ and $\sum_{u\in U} \sigma_\epsilon(h)(u) = 1$.
\end{itemize}
...
\textbf{Step 3.}
Using $\theta_\emph{Max}^{ml}$, we construct a DFA $\mathcal{A}$ encoding the desired regular \hbox{$\epsilon$-optimal} strategy $\sigma_\epsilon^{reg}$ as follows: $\mathcal{A} = (Q, U, \delta, q_0, \{\})$ where
\vspace{-3pt}
\begin{itemize}
...
%For a commitment $f$, we define $\xi(f) = f(r(f))$ to be the probability distribution in the root of $f$.
...
%TODO zmenit nazov?
%\section{Reducing the State Space} / The Characteristics?
...
\end{definition}
The values for $k = 0$ contain the information about the current location of the defender and we put\\
\[\mathbf{Char}_p(0, u) =
   \begin{cases}
   1		& \text{if } r(p) = u \\
   0		& \text{otherwise.}
   \end{cases}
\]
%TODO inductive definition - slovna definicia sa zide, ked budem d-commitmentom priradovat charakteristiky
For $1 \leq k \leq d$ we put
$$\mathbf{Char}_p(k, u) = \xi(p)(u) + \sum_{v \neq u}\xi(p)(v) \cdot \mathbf{Char}_{\delta(p, v)}(k - 1, u)~.$$
Note that the values for $k = 1$ represent the probability distribution over the nodes in the next step as $\mathbf{Char}_{\delta(r, u)}(0, v) = 0$ for all $v \neq u$.
...
%      \end{cases}
%		& \text{if } k = 0 \\
%   \xi(p)(u) + \sum_{v \neq u}\xi(p)(v) \cdot \mathbf{Char}_{\delta(p, v)}(k - 1, u)	& \text{otherwise.}
%   \end{cases}
%$
%
%\noindent$
...
Given $q \in Q \smallsetminus \{q_0\}$, we denote by $\mathcal{A}_{p \leftarrow q} = (Q', U, \delta', q_0, \{\})$ the DFA defined as follows:
\vspace{-5pt}
\begin{itemize}
...
%However, we prove that the states' characteristics are preserved if the two merged states have the same characteristics.
However, the states' characteristics are preserved if the two merged states have the same characteristics, as is formulated in the following lemma.
...
where $\mathbf{Char}'_r$ denotes the characteristics of $r$ in the automaton $\mathcal{A}_{p \leftarrow q}$.
\end{lemma}
...
\begin{align}
\mathbf{Char}'_r(k, v)
  &= \xi(r)(v) + \sum_{u \neq v}\xi(r)(u) \cdot \mathbf{Char}'_{\delta'(r, u)}(k - 1, v)\label{eq2:first} \\
  &= \xi(r)(v) + \sum_{u \neq v}\xi(r)(u) \cdot \mathbf{Char}_{\delta'(r, u)}(k - 1, v)\label{eq2:second} \\
  &= \xi(r)(v) + \sum_{u \neq v}\xi(r)(u) \cdot \mathbf{Char}_{\delta(r, u)}(k - 1, v)\label{eq2:third} \\
  &= \mathbf{Char}_r(k, v)\label{eq2:fourth}~.
\end{align}
\vspace{-20pt}
\begin{itemize}
	\item The equalities \eqref{eq2:first} and \eqref{eq2:fourth} follow directly from the definition of the characteristics. %and the fact that $\mathbf{Char}_{\delta(r, u)}(0, v) = 0$ for all $u \neq v$.
...
Let us fix and denote by $\mathcal{A}_{red}$ %= (Q_{red}, U, \delta_{red}, q_0, \{\})$
...
\begin{definition}
Given a state $p \in Q$, we define the value of its characteristics by $$val(\mathbf{Char}_p) = \min_{t \in T} \mathbf{Char}_p(d, t)~.$$
\end{definition}
...
We can further substitute each state $p \in Q_{red}$ by its characteristics $\mathbf{Char}_p$ to obtain an automaton $\mathcal{A}_{Char}$.
%One can easily prove that $\mathcal{A}_{Char}$ and $\mathcal{A}_{red}$ encode exactly the same defender's %strategy.
%It suffices to put
By putting
\vspace{-5pt}
\begin{itemize}[label=-]
%	\item $Q_{Char} = \{\mathbf{Char}_p \mid p \in Q_{red}\}$,
	\item $\delta_{Char}(\mathbf{Char}_p)(u) = \mathbf{Char}_q$ iff $\delta_{red}(p)(u) = q$, and
	\item $\sigma_{Char}(h)(u) = \mathcal{A}_{Char}[h](1, u)$ %$ = \xi(\mathcal{A}_{red}[h])(u)$
\end{itemize}
\vspace{-5pt}
one can easily see that $\sigma_{Char} = \sigma_{red}$.

%TODO delete? dal som to k zdovodneniu pre "representable" charakteristik
%\noindent
%\textit{Remark 1. }
%When a play is in a state $p \in Q_{Char}$, there is exactly one unique $d$-commitment $f$ (determined by $\sigma_{Char}$) which represents the current $d$-step randomized plan for the defender.
...
So far, we have shown that there is a DFA $\mathcal{A}_{Char}$ encoding an $\epsilon$-optimal strategy such that its states $Q_{Char}$ are mutually different characteristics.
Furthermore, each $c \in Q_{Char}$ satisfies:
\vspace{-5pt}
\begin{enumerate}[label=(\Alph*)]
	\item $c = \mathbf{Char}_c$\footnote{We identify a state with its characteristics to reflect that it is the only quality of a state we are interested in.} \label{a}
	\item $c(1, u)$ is an integer multiple of $t$ for all $u \in U$ \label{b}
...
%	\item no two states have the same characteristics, and
%	\item the value of the characteristics of each state is at least $val(\sigma) - \epsilon$.
\end{enumerate}
\vspace{-5pt}
In order to find such an automaton, we consider the set of all discretized characteristics
$\emph{Char}^t$ that consists of all characteristics $c$ such that $c(k, u)$ is an integer multiple of $t^k$ for all $0 \leq k \leq d$ and $u \in U$.
From \ref{a} and \ref{b} it follows that $Q_{Char} \subseteq \emph{Char}^t$.
...
%TODO characteristics - singular or plural?
Each discretized characteristics represent a possible state of the automaton.
...
Therefore, we consider only characteristics that are \emph{representable} by some $d$-commitment---otherwise, they could not be the characteristics of any state.
The problem of how to determine whether characteristics are representable will be addressed later.
%TODO is representable!
%vobec netreba, aby boli charakteristiky REPRESENTABLE! staci, ak budu legalne v prvom kroku

Let $U = \{u_1, \dots, u_n\}$ be the set of nodes of $\mathcal{G}$ %TODO $|U| = n$
and let $c, c_1, \dots, c_n \in Char^t$ be some discretized characteristics (not necessarily different).
We say that $\{c_1, \dots, c_n\}$ is a set of \emph{successors} of $c$ if
$c(k, u) = \sum_{i = 1}^n c(1, u_i) \cdot c_i(k - 1, u_i)$
for all $1 \leq k \leq d$ and $u \in U$.
The meaning of this is that if, for all $i$, $c_i = \mathbf{Char}_{c_i}$ and there is a transition from state $c$ to $c_i$ under $u_i$, then also $c = \mathbf{Char}_{c}$.

We say that a non-empty set $E \subseteq Char^t$ is \emph{closed} if for all $c \in E$ there is a set $S \subseteq E$ of successors of $c$. %if ... there is a set $S$ of successors of $c$ such that $S \subseteq E$.
Finally, we say that a set $D \subseteq Char^t$ is \emph{complete} if it contains a closed subset.

%TODO
Note that each closed set $E$ represents a set of states of an automaton, whose transition function can be built from the states' successors (not deterministic).
\\ $Q = E$
\\ $q_0 = c$ such that $c(0, \hat{u}) = 1$ (there has to be such $c$ unless the value of all characteristics from $E$ is 0)
\\ $\sigma(c, u_i) = c_i$ where $\{c_1, \dots, c_n\}$ is a set of successors of $c$ such that $c_i(0, u_i) = 1$
\\ We prove that this automaton determines an $\epsilon$-optimal strategy.

%proof of $c = \mathbf{Char}_c$: by induction on $k$ that for all characteristics $c$ and all nodes $u$ it holds that $c(k, u) = \mathbf{Char}_c(k, u)$

%TODO $x \ leq k \ leq y$  nahradit $k \in \{x, \dots, y\}$
...
The main reason behind choosing this model is that we are interested in the "most" optimal strategy---any unnecessary restrictions might lower the value of the game.
Example \ref{ex:centralized} shows a game where a joint strategy yields strictly larger value than any disjointed strategy.

The second reason is simplicity---assigning a specific set of nodes to each robot would significantly complicate the model.
...
We define \emph{configuration} to be an $|R|$-tuple of nodes specifying the positions of the robots \cite{multi-robot}.
We write $c = (c_1, \dots, c_{|R|})$, where $c_i \in U$ is the position of the robot $r_i \in R$.
...
Using this, we straightforwardly modify the definitions of \emph{path}, \emph{history}, and \emph{run} from the patrolling game to work with configurations instead of single nodes.
...
The attacker cannot attack before the robots are placed in the initial configuration as he could easily take advantage of this information (see Example \ref{ex:distribution}).
Note that although the defender has multiple patrolling units, the attacker can still attack only a single node and only once during a play.
...
%However, a setting with $supp(\sigma(\epsilon))$ not being a singleton seems unrealistic.
\end{remark}

%robots are homogeneous
%vsetci sa hybu rovnako rychlo

%-> consistent with the underlying subgraph

%TODO zadefinovat successful attack a pod.
...
\begin{tabular}{ m{1cm} m{5cm} m{6cm} }
...
\begin{tikzpicture}
  \foreach \pos/\name in {{(0,0)/a}, {(1.5,0)/b}, {(3,0)/c}, {(3,1.5)/d}, {(1.5,1.5)/e}, {(0,1.5)/f}}
        \node (\name) at \pos {$\name$};
...
\vspace{-5pt}
\caption{{Simple settings for examples of multi-robot patrolling games.}}
\label{fig:examples2}
\end{figure}

\begin{example}
\label{ex:centralized}

Fig.~\ref{fig:examples2} (left)
\end{example}

\begin{example}
\label{ex:initial2}

Fig.~\ref{fig:examples2} (right)
\end{example}
...
%TODO charakteristika - zadefinovat bez stavu; a potom 2. definicia so stavom, v ktorej spojim slovnu definiciu s induktivnou			- taktiez najst ine angl. slovo pre slovenske "charakteristika"
%TODO successors => sequence (c_1, ..., c_n su successormi), prip. c_i je u_i-successor of c
