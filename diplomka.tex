...
%TODO preformulovat ?
Further, when we realize the invariant $0 \leq \sum_{j=1}^{i-1}(\sigma(h)(u_j) - \sigma_\epsilon(h)(u_j)) < c$ holds for all $1 \leq i \leq |U|$, it is easy to see that $|\sigma(h)(u_i) - \sigma_\epsilon(h)(u_i)| < c$.% (for all $i$)
\begin{proposition}
...
Denote by $R[\iota, u, t, k]$ the probability of reaching $t \in T$ from $u \in U$ in up to $k$ steps using the strategy $\iota$.
Note that
$$att\text{-}val(\iota \mid h) = \inf_{t \in T} R[\iota_h, last(h), t, d]~.$$
Using Proposition \ref{prop:difference}, we prove by induction on $k$ that
\begin{align}
| R[\sigma_h, last(h), t, k] - R[{\sigma_\epsilon}_h, last(h), t, k]| \leq k|U|c\label{eq1:induction}
\end{align}
and therefore
$$| att\text{-}val(\sigma_\epsilon \mid h) - att\text{-}val(\sigma \mid h)| \leq d|U|c \leq \epsilon~.$$

Now, we prove that the inequality \eqref{eq1:induction} holds for all $h \in \mathcal{H}$, $t \in T$, and $k \in \mathbb{N}$.
The base case ($k = 1$) directly follows from Proposition \ref{prop:difference} and the fact that $R[\iota_h, last(h), u, 1] = \iota(h)(u)$.
...
%TODO zmazat
\begin{proof}
%$val(\sigma_\epsilon) \geq \inf_{h \in \mathcal{H}(\sigma_\epsilon)} att\text{-}val(\sigma_\epsilon \mid h) \geq val(\sigma) - \epsilon$
\end{proof}
