...
In this chapter, we generalize the model of patrolling games introduced in Chapter \ref{chap:pg} by allowing multiple patrolling units (robots).
...
We chose to study the most general model, which corresponds to \emph{joint strategy} (one strategy regarding the movement of all the robots is computed centrally) with \emph{full assignment} (each robot can potentially visit every vertex).
...
Example \ref{ex:centralized} shows a game where a joint streategy yields strictly larger value than any disjointed strategy.

The second reason is simplicity---assigning a specific set of vertices to each robot would significantly complicate the model.
...
%Section 3.1 - az po {Definition} mi nic netreba - slova/abeceda/... vsetko pasuje i pre multi-robot
- configuration - an |R|-tuple of vertices specifying the positions of robots.
- C - configuration space
-> consistent with the underlying subgraph
- history

- joint strategy, full assignment
...
%mozno mozeme povolit, aby mohol zautocit aj hned - strategia to zvladne bez znizenia hodnoty hry
