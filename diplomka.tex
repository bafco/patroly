...
%TODO - presunut dokaz niekam inam?
\begin{proof}
...
\begin{align}
R[\sigma, h, t, k] - R[\sigma_\epsilon, h, t, k] \leq k|U|c~.\label{eq1:induction}
\end{align}

%The base case ($k = 1$) directly follows from Lemma \ref{lem:difference} and the fact that $R[\iota, h, u, 1] = \iota(h)(u)$.
The base case ($k = 0$) clearly holds as the value $R[\iota, h, u, 0]$ is independent of the strategy $\iota$ --- it is $1$ if $last(h) = u$, and it is $0$ otherwise.

For $k \geq 1$, we have
\begin{align}
%TODO double-check once it is splitted across multiple lines
R[\sigma, h, t, k] - R[\sigma_\epsilon, h, t, k]
  &= \sum_{u\in U}\sigma(h)(u)\cdot R[\sigma, hu, t, k - 1]
- \sigma_\epsilon(h)(u)\cdot R[\sigma_\epsilon, hu, t, k - 1]	\label{eq1b:first} \\
  &= \sum_{u\in U} R[\sigma, hu, t, k - 1]\cdot (\sigma(h)(u) - \sigma_\epsilon(h)(u))
+ \sigma_\epsilon(h)(u))\cdot (R[\sigma, hu, t, k - 1] - R[\sigma_\epsilon, hu, t, k - 1])	\label{eq1b:second} \\
  &\leq \sum_{u\in U} R[\sigma, hu, t, k - 1]\cdot c
+ \sum_{u\in U} \sigma_\epsilon(h)(u)\cdot (k - 1)|U|c	\label{eq1b:third} \\
  &\leq |U|c + (k - 1)|U|c	\label{eq1b:fourth} \\
  &\leq k|U|c\label{eq1b:fifth}~.
\end{align}

% It follows from the definition of $R[\iota, h, t, k]$ that 
- By definition, we have $R[\iota, h, t, k] = \sum_{u \in U} \iota(h)(u) \cdot R[\iota, hu, t, k - 1]$.

%From Lemma \ref{lem:inequality}
%TODO
