...
\begin{enumerate}[label=(\Alph*)]
	...
	\item $c(1, u)$ is an integer multiple of $s$ for all $u \in U$ \label{b}
...
$\emph{Char}^s$ that consists of all characteristics $c$ such that $c(k, u)$ is an integer multiple of $s^k$ for all $0 \leq k \leq d$ and $u \in U$.
From \ref{a} and \ref{b} it follows that $Q_{Char} \subseteq \emph{Char}^s$.
...
and let $c, c_1, \dots, c_n \in Char^s$ be some discretized characteristics (not necessarily different).
...
We say that a non-empty set $E \subseteq Char^s$ is \emph{closed} if for all $c \in E$ there is a set $S \subseteq E$ of successors of $c$. %if ... there is a set $S$ of successors of $c$ such that $S \subseteq E$.
Finally, we say that a set $D \subseteq Char^s$ is \emph{complete} if it contains a closed subset.
