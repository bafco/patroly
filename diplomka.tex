...
In this chapter we present the basics of game theory. The definitions and content used here are inspired by \cite{polak} and \cite{gameTheory}.

Game theory is a mathematical study of decision making. It studies games as well-defined mathematical object.
A game is a played between players $P_1, \dots, P_n, n \geq 2$.
Each player $P_i$ has a non-empty set $X_i$ of possible \emph{strategies}.
The outcome of a game is an element from the set $\prod_{i=1}^n S_i$ and is called a \emph{situation}.
Each situation represents some value for every player.
Therefore, every player $P_i$ has his own \emph{utility function} $U_i: \prod_{i=1}^n S_i \mapsto \mathbb{R}$ which assigns \emph{payoffs} to situations.
The higher the player's payoff of a situation, the better the situation for the player.
The above representation of a game is called normal form and is mostly used when a strategy represents a single action.

The extensive form of a game is appropriate to formalize games with time sequencing of moves.
