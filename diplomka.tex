...
patrolling games, game theory, complexity, algorithm, optimal strategy, $\epsilon$-optimality, oriented graph, multiple robots, multi-robot, characteristic, Stackelberg model
\end{ThesisKeyWords}
...
\hline
\cite{multi-robot} & no	& stochastic	& yes	& yes & targets have different values \\
\hline
...
For example, we still do not know whether, in general, the automaton's state-space can be bounded above by the number of different $1$-commitments, i.e. no two states would have the same probability distributions in their root. However, we believe that this cannot be done.
Inspecting the properties of optimal strategies might reveal new ways for more efficient algorithms.
Finally, some advanced results from other fields might significantly reduce the time needed to determine whether a set of characteristics is complete.
