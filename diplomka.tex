...
%\cite{iti2}
%A defender’s strategy $\sigma$ is regular if there is a deterministic finite-state automaton $\mathcal{A}$ over the alphabet U (the nodes) such that $\sigma(h)$ depends only on the control state entered by $\mathcal{A}$ after reading h. Hence, $\mathcal{A}$ gathers some finite information about the history of a play which can be used in decision making. Regular strategies are widely used in algorithmic game theory because they are precisely the strategies implementable by finite-memory controllers.

\
%TODO uviest jednotlive kroky + o co sa jedna

\textbf{Step 1.} Let us fix an optimal strategy $\sigma$, whose existence is guaranteed by Theorem \ref{thm:existence}.
%For a desired precision $\epsilon$
We discretize $\sigma$ to obtain a strategy $\sigma_\epsilon$ with all probabilities $\sigma_\epsilon(h)(u)$ being integer multiples of an appropriate number $t$ satisfying \mbox{$t = 1/k$} for some $k \in \mathbb{N}$.
%TODO
As we will see, to guarantee the $\epsilon$-optimality of $\sigma_\epsilon$ ...

%TODO rounding procedure
Let $U = \{u_1, u_2, \dots, u_{|U|}\}$ be the set of nodes of $\mathcal{G}$.
For every history $h$ and every $1 \leq i \leq |U|$, we inductively define $\sigma_\epsilon(h)(u_i) = k_i \cdot t$, where $k_i$  is the 
%smallest number satisfying $$\sigma(h)(u_i) - k_i \cdot t  $$
largest number satisfying
...
that $\sigma_\epsilon(h)$ is indeed a probability distribution over $U$, i.e. $\sum_{u\in U}\sigma_\epsilon(h)(u) = 1$ (note that simple rounding cannot guarantee this property).
%TODO preformulovat
...
%TODO je to strategia - v sucte dava 1
...
\textbf{Step 3.}
Using $\theta_\emph{Max}^{ml}$, we construct a finite-state automaton $\mathcal{A}$ for the desired regular $\epsilon$-optimal strategy $\sigma_\epsilon^{reg}$ as follows: $\mathcal{A} = (Q, U, \delta, q_0, \{\})$ where
\vspace{-5pt}
\begin{itemize}[noitemsep]
	\item $Q = \{f \mid (\emph{Max}, f) \in \mathbf{Reach}(\theta_\emph{Max})\} \cup {q_0}$ is the finite set of states,
...
$\delta(f, u) = f'$ where $f'$ is the $u$-suffix of $\theta_\emph{Max}^{ml}((\emph{Max}, f))$,\footnote{Note that $\theta_\emph{Max}^{ml}$ takes as an argument only a single vertex. That would not be possible unless it was memoryless.} %IDEA footnote napisat niekam inam? (ako remark?)
	\item $q_0$ is the initial state.

\end{itemize}
\vspace{-5pt}
%TODO vybrat len jedno z A[h](last(h)) a f(r(f)) ?
We define $\sigma_\epsilon^{reg}(h) = \mathcal{A}[h](last(h)) = f(r(f))$ where $f = \mathcal{A}[h]$ is the state of $\mathcal{A}$ reached after reading the history $h$.
It is easy to show that $\sigma_\epsilon^{reg}(h)$ is the same as $\sigma[\theta_\emph{Max}^{ml}]$ obtained from $\theta_\emph{Max}^{ml}$ using Lemma \ref{lem:translation}.
It follows that
