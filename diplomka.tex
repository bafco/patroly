...
Particuraly note that $\sigma_{\sigma(\epsilon)} = \sigma$.
...
\textbf{Step 1.} Let us fix an optimal defender's strategy $\sigma$, whose existence is guaranteed by Theorem \ref{thm:existence}.
For a desired precision $\epsilon$, we discretize $\sigma$ to obtain a strategy $\sigma_\epsilon$ with all probabilities $\sigma_\epsilon(h)(c)$ being integer multiples of an appropriate number $s$ satisfying \mbox{$s = 1/l$} for some $l \in \mathbb{N}$.
As we will see, to guarantee the $\epsilon$-optimality of $\sigma_\epsilon$, it suffices that $|\sigma(h)(c) - \sigma_\epsilon(h)(c)| \leq {\epsilon\over{|C|d}}$.
We can guarantee this property for $s = \lceil(|C|d)/\epsilon\rceil^{-1}$ using the following rounding procedure.

Let $C = \{c^1, c^2, \dots, c^{|C|}\}$ be the set of all configurations.
For every history $h$ and every $1 \leq i \leq |C|$, we inductively define $\sigma_\epsilon(h)(c_i) = k_i \cdot s$, where $k_i$  is the largest number satisfying
$$ k_i \cdot s \leq \sigma(h)(c^i) + \sum_{j=1}^{i-1}(\sigma(h)(c^j) - \sigma_\epsilon(h)(c^j))~.$$
This rounding procedure guarantees
that $\sigma_\epsilon(h)$ is indeed a probability distribution over $C$, i.e. $\sum_{c\in C}\sigma_\epsilon(h)(c) = 1$ (note that simple rounding would not guarantee this property).
Further, when we realize the invariant $0 \leq \sum_{j=1}^{i-1}(\sigma(h)(c^j) - \sigma_\epsilon(h)(c^j)) < s$ holds for all $1 \leq i \leq |C|$, it is easy to see that $|\sigma(h)(c^i) - \sigma_\epsilon(h)(c^i)| < s$, which is captured by the following lemma.

\begin{lemma}
\label{lem:differenceMRPG}
For all $h \in \mathcal{H}$ and $c \in C$ we have $|\sigma(h)(c) - \sigma_\epsilon(h)(c)| < s$.
\end{lemma}
...
\begin{itemize}
	...
	\item To prove the first inequality \eqref{eq4:first}, we have to analyze the impact of the rounding in the definition of $\sigma_\epsilon$.
