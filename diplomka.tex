...
Realizing %the discretization procedure used to define the strategy $\sigma_\epsilon$ (see Step 1 in Section \ref{sec:discretizationMRPG}) and the fact
that values $\sigma_{red}(h)(c)$ are integer multiples of $s$, it is easy to see that $Q_\mathit{Char} \subseteq \mathit{Char}^s$.
...
\vspace{-5pt}
\begin{itemize}
	\item if $\xi(f)(c_i) > 0$, then $r(f_i) = c_i$,\footnote{blabla we cannot ignore the 'if' clause}
...
As in Section \ref{sec:characteristics}, we say that a non-empty set $B \subseteq \mathit{Char}^s$ is \emph{closed} if for every $\gamma \in B$ there are successors $(\gamma_1, \dots, \gamma_{|C|}) \in B^{|C|}$ of $\gamma$.
Finally, we say that a set $D \subseteq \mathit{Char}^s$ is \emph{complete} if it contains a closed subset.
...
Contrary to the SRPGs, the initial state of the automaton can be any of its states as an MRPG does not force a specific initial configuration.
...
We define the strategy $\sigma_B$ determined by $\mathcal{A}_B$ as
...
m(k, u)
  ...
  &= \sum_{c \in C_u}\xi(f)(c) + \sum_{i = 1, c_i \notin C_u}^{|C|} \xi(f)(c_i)\cdot m_{\gamma_i}(k-1, u)	\label{eq6:second} \\
  &= \sum_{c \in C_u}\xi(f)(c) + \sum_{i = 1, c_i \notin C_u}^{|C|} \xi(f)(c_i)\cdot m_{\delta_B(\gamma, c_i)}(k-1, u)	\label{eq6:third} \\
  &= \sum_{c \in C_u}\xi(f)(c) + \sum_{c \in C \smallsetminus C_u} \xi(f)(c)\cdot m_{\delta_B(\gamma, c)}(k-1, u)	\label{eq6:fourth} \\
  &= m_\gamma(k, u)\label{eq6:fifth}~.
\end{align}
The equality \eqref{eq6:second} follows from the induction hypothesis (we used notation $\mathbf{Char}_{\gamma_i} = (f_{\gamma_i}, m_{\gamma_i})$).
