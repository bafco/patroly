...
%	\item the characteristics of all the states to which there is a transition from $r$ under particular node $u$.
%\end{itemize}
%\vspace{-20pt}
\
...
Formally, we prove by induction on $k$ that for all $r \in Q \smallsetminus \{q\}$, $v \in U$, $1 \leq k \leq d$ we have that $\mathbf{Char}'_r(k, v) = \mathbf{Char}_r(k, v)$.
All we need is to realize that $\xi(r)$ is the same in $\mathcal{A}$ and $\mathcal{A}_{p \leftarrow q}$.
%for all $r \in Q \smallsetminus \{q\}$.
...
\begin{itemize}[noitemsep, label=--]
	\item The equalities \eqref{eq:first} and \eqref{eq:fourth} follow directly from the definition of characteristics.
