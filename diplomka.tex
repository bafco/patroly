...
It follows from \ref{a} and \ref{b} that $Q_{Char} \subseteq \emph{Char}^t$.
...
\Repeat
	\State pull from $C$ the item $c$ with highest priority
	\State $D \leftarrow D \cup \{c\}$
\Until {$D$ is \emph{closed}}	%{$closed(D)$}
\State $\sigma_\epsilon^{alg} \leftarrow$ the strategy induced by ... %TODO
\end{algorithmic}
\end{algorithm}

The algorithm we provide to compute the desired $\epsilon$-optimal strategy is a modification of the algorithm from \cite{iti1}---instead of searching the set of discretized $d$-commitments, we search the set of discretized characteristics.
