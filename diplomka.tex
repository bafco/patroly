...
We later use this knowledge to prove that an appropriately discretized strategy (for a given precision $\epsilon$) has value at least $val(\sigma) - \epsilon$ throughout the play, and therefore is $\epsilon$-optimal.
...
For $C = U^z$ and $s = \lceil(|C|d)/\epsilon\rceil^{-1}$ we get that the Algorithm \ref{alg:algorithm2} runs in time $\left(\frac{d\cdot|U|^z}{\epsilon}\right)^{\mathcal{O}(|U|^{2z} + d^2\cdot|U|^{z+1})}$, which proves Theorem \ref{thm:complexityMRPG}.
...
%- number of 1-commitments when all except root and \xi is ignored
