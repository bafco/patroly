...
A noticeably novel game model called MRMT\textsubscript{sg} (multiple Mobile Resources protecting Moving Targets) was introduced in \cite{line}. In MRMT\textsubscript{sg}, the defender's strategy space is discretized and attacker's strategy space is continuous. A major motivation in the paper is the problem of protecting ferries with fast patrol boats.
...
\end{proof}
\noindent
%Now we proceed in the same way as in Section \ref{sec:characteristics}.
...
As for the Algorithm \ref{alg:algorithm1}, the decisive part of the execution time is spent in determining whether a set $D$ is complete.
...
As $D \subseteq \mathit{Char}^s$, it remains to find an upper bound for $|\mathit{Char}^s|$, i.e. determine how many different discretized characteristics $(f, m)$ exist.
The number of possible values for $r(f)$ is $|C|$, for $\xi(f)$ it is $(s^{-1})^{|C|}$, and for $m$ it is $(s^{-1})^{d^2|U|}$.
For $C = U^z$ and $s = \lceil(|C|d)/\epsilon\rceil^{-1}$ we get that the Algorithm \ref{alg:algorithm2} runs in time $\left(\frac{d\cdot|U|^z}{\epsilon}\right)^{\mathcal{O}(|U|^{2z} + d^2\cdot|U|^{z+1})}$.
...
%find some lower bound on the number of needed checkes whether a tuple of characteristics are successors of another characteristic
