...
The algorithm runs in time polynomial in the number of all discretized $d$-commitments, i.e. in time ${\left({{|U|d}\over \epsilon}\right)}^{|U|^{\mathcal{O}(d)}}$.
...
\end{itemize}
\vspace{-5pt}
%The unique node $u$ is called the \emph{root} of the characteristic $c$ and is denoted by $r(c)$.
\end{definition}
\noindent
%In the following definition, we assume the automaton $\mathcal{A}$ constructed in Step~3 in the previous section.
\begin{definition}
%TODO
Let $\mathcal{A} = (Q, U, \delta, q_0, \{\})$ be an automaton encoding a defender's strategy.
...
\end{definition}
\noindent
The values for $k = 0$ contain the information about the current location of the defender
...
\begin{itemize}
	\item
\hspace{27.4pt}$\mathbf{Char}_p(0, u) =
   \begin{cases}
   1		& \text{if } r(p) = u \\
   0		& \text{otherwise}
   \end{cases}$
...
Observe that $\mathbf{Char}_p(1, u) = \xi(p)(u)$, because $\mathbf{Char}_{\delta(p, v)}(0, u) = 0$ for all $v \neq u$.
...
\vspace{-20pt}
\begin{itemize}
	\item The equalities \eqref{eq2:first} and \eqref{eq2:fourth} follow directly from the definition of the characteristic. %and the fact that $\mathbf{Char}_{\delta(r, u)}(0, v) = 0$ for all $u \neq v$.
...
We can further substitute each state $p \in Q_{red}$ by its characteristic $\mathbf{Char}_p$ to obtain an automaton $\mathcal{A}_{Char}$.
...
We say that a non-empty set $E \subseteq Char^s$ is \emph{closed} if for all $c \in E$ there is a set $S \subseteq E$ of successors of $c$.
%if ... there is a "sequence" $(c_1, \dots, c_n) \in E^n$ of successors of $c$
...
\\ $Q = E$ %TODO premenovat E na nieco ine, nech sa to nemyli s Edges
...
\\
\\ strategy $\sigma_\epsilon^{Alg} = \xi(\mathcal{A}[h^+])$, where $\xi(c)$ is the probability distribution in the root of $c$ defined by $\xi(c)(u) = c(1, u)$
\\ We prove that this automaton determines an $\epsilon$-optimal strategy.
It suffices to prove that for all $c \in Q$ we have that $c = \mathbf{Char}_c$ and that $val(c) \geq val(\sigma) - \epsilon$.

We prove by induction on $k$ that for all $c \in Q$ and $u \in U$ it holds that $c(k, u) = \mathbf{Char}_c(k, u)$.
Let $\{c_1, \dots, c_n\}$ be the set of successors of $c$.

For $k = 0$ this trivially holds.


%TODO bude to treba cele prekopat a zadefinovat regularnu strategiu ako je v charakteristiky.pdf

For $1 \leq k \leq d$, we have
\begin{align*}
c(k, u)
  &= \sum_{i = 1}^n c(1, u_i) \cdot c_i(k - 1, u_i) \\ %from the definition of successors
  &=  \\ %
  &=  \\ % from the definition of characteristic of a state
  &= \mathbf{Char}_c(k, u)~.
\end{align*}

$c(k, u) = \sum_{i = 1}^n c(1, u_i) \cdot c_i(k - 1, u_i)$
$= $
From the definition of the characteristic, we have
$$\mathbf{Char}_c(k, u) = \xi(c)(u) + \sum_{v \neq u}\xi(c)(v) \cdot \mathbf{Char}_{\delta(c, v)}(k - 1, u)$$
where $\xi(p)$ is the probability distribution in the root of $p$
\\ - for a $d$-commitment $p$: $\xi(p)(u) = p(r(p))(u)$
\\ - for a characteristic $p$: $\xi(p)(u) = {Char}_p(1, u)$
