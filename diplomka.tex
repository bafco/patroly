...
%Recall that $val_u(\sigma) = \emph{inf}_{\pi \in \Pi}\mathcal{P}_u^{\sigma}(\mathcal{D}_u[\pi])$.
...
A defender’s strategy $\sigma$ is \emph{regular} if there is a deterministic finite-state automaton $\mathcal{A}$ over the alphabet $U$ (the nodes) such that $\sigma(h)$ depends only on the control state entered by $\mathcal{A}$ after reading $h$ \cite{iti2}.
...
Third, using $\theta_\emph{Max}^{ml}$, we construct a finite-state automaton which encodes an $\epsilon$-optimal strategy.
...
\textbf{Step 3.}
Using $\theta_\emph{Max}^{ml}$, we construct a finite-state automaton $\mathcal{A}$ for the desired regular $\epsilon$-optimal strategy $\sigma_\epsilon^{reg}$ as follows: $\mathcal{A} = (Q_0, U, \delta, q_0, \{\})$ where
\vspace{-3pt}
\begin{itemize}[noitemsep]
...
%TODO potrebujem mat zadefinovane $att-val$ - malo by byt v ramci dokazu $\epsilon$-optimality
$att\text{-}val(\iota \mid h) = \text{inf}_{t \in T} \mathcal{P}_{last(h)}^{\iota_h}(D_t)$
\begin{lemma}
