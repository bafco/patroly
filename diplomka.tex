...
\begin{ThesisThanks}
I would like to express my sincere gratitude to my advisor, doc. RNDr. Tomáš Brázdil, Ph.D., for his valuable advices, patience, help and time, which he gave me even during weekends.
Further, I would like to thank RNDr. Vojtìch Øehák, Ph.D. for consultations and clarifications regarding the topic of this thesis.
And last but not least, I am grateful to my brother, Mgr. Michal Abaffy, for persuading me to work on such a challenging topic.
\end{ThesisThanks}

\begin{ThesisAbstract}
Patrolling games are two-player games played on graphs.
One player, the attacker, aims at attacking some of the nodes, while the other player, the defender, tries to protect those nodes by visiting them.
The defender has to randomize as her strategy and position are both known to the attacker.

We present a quadratically exponential algorithm which computes an $\epsilon$-optimal defender's strategy for a given patrolling game and a given precision $\epsilon$.
Further, we extend the model of patrolling games by allowing multiple patrolling robots.
We present an algorithm to compute an $\epsilon$-optimal defender's strategy in doubly exponential time in the number of robots.

\end{ThesisAbstract}

\begin{ThesisKeyWords}
patrolling games, game theory, complexity, algorithm, optimal strategy, $\epsilon$-optimality, oriented graph, multiple robots, characteristic, Stackelberg model
\end{ThesisKeyWords}
...
Patrolling games have become a topic of increasing interest since the patrolling concept was introduced in \cite{perimeter} in 2008.
Patrolling games are security-oriented games played between two players, the \emph{defender} and the \emph{attacker}, mostly on an oriented graph.
Some of the nodes, called \emph{targets}, are valuable to the players and are subject to attacks.
The attacker needs some finite \emph{penetration time} to successfully complete an attack.
During this time, he cannot move and can be caught by the defender.
The defender's goal is to protect the targets from intrusions by moving a mobile robot (or robots) through them.
As it is common in security games, the attacker is supposed to know the defender's position and strategy.
This leads to adopting the Stackelberg model, in which, informally said, the defender's objective is to minimize the harm in the worst case scenario.

Many different models of patrolling games have been already defined and studied.
Some of them are covered in more detail in Section $\ref{sec:patrolLiterature}$.
The model we adopted is from \cite{miso}.
We model a patrolling game as a zero-sum (or strictly competitive) game.
The game is played on a directed graph, which represents the possible locations and moves of the defender.
Some of the nodes are valuable targets.
The defender starts in some target and moves along the edges, trying to protect the targets.
Each step takes one time unit.
The defender chooses the next node randomly and this choice may be based on the history of all the nodes already visited.
In each step, the attacker chooses whether to attack some target, or wait.
To successfully complete an attack, the attacker needs to be in a target undetected for $d$ steps.
Once the attack is detected or completed, the play of the game ends.

Our model is simple in the assumptions about the targets, particularly that all the targets are considered to be equal.
There is only a single penetration time $d$ common for all the targets and the players have no preference over the targets, i.e. all have the same value.
There is one virtue specific to our model which is not covered by any other papers prior to \cite{miso} and that is the infinite-memory strategy.
It has been proven in \cite{iti2} that infinite-memory strategies may result in higher expected utility for the defender than any finite-memory strategies.

The patrolling model defined in \cite{miso} has been studied deeper in \cite{iti1}, where many conjectures from \cite{miso} have been proven and stronger results have been achieved.
We build this thesis on the results from \cite{iti1}\footnote{\cite{iti1} is only an extended abstract and when we cite this paper, we usually refer to the unpublished technical part available online at $<$http://goo.gl/ueKS79$>$.}, following their definitions and techniques.

The thesis is structured as follows.
In Chapter \ref{chap:preliminaries}, we present the basics of game theory and we introduce security games and patrolling games in general.
We discuss our model, as well as other models, in more detail.
In Chapter \ref{chap:pg}, we formally define a patrolling game and provide examples for a better understanding of our model.
Chapter \ref{chap:existence} summarizes the results from \cite{iti1} where an optimal defender's strategy has been proven to exist for an arbitrary patrolling game.
In Chapter \ref{chap:main}, we extend the results from \cite{iti1} by providing a quadratically exponential algorithm for computing an $\epsilon$-optimal defender's strategy.
Finally, in Chapter \ref{chap:multiPatrol}, we extend the patrolling model by allowing multiple robots to protect the targets.
We present a doubly exponential algorithm to compute an $\epsilon$-optimal defender's strategy.
That is, to the best of our knowledge, the first algorithm which can compute an $\epsilon$-optimal (infinite-memory) strategy for the defender in a multi-robot setting.
...
A \emph{game} is played between \emph{players} $P_1, \dots, P_n, n \geq 2$.
Each player $P_i$ has a non-empty set $S_i$ of \emph{pure strategies}. A strategy can be seen as an algorithm telling the player what to do throughout a play of the game.
...
The above representation of a game is called normal (or strategic) form. The normal game is usually represented by a matrix and is mostly used when a strategy represents a single action.
...
Non-zero sum games are referred to as \emph{general-sum} games.
...
Different solution concepts were designed for specific game models. We are especially interested in the Stackelberg model, which we describe in the next section.

\section{Security Games}

Security games are two-player games in which \emph{defender} allocates resources to guard targets against an \emph{attacker} (also called \emph{adversary} or \emph{intruder}), who aims to attack some target.\footnote{In the rest of the paper, we refer to the defender as "she" and to the attacker as "he".}
...
\subsection{Security Games in Real World}
...
The second application, deployed by the Federal Air Marshals Service (FAMS), is called \emph{IRIS} (Intelligent Randomization In Scheduling) \cite{iris}.
...
In contrast with ARMOR and IRIS, which focused on one security activity, GUARDS reasons about hundreds of heterogeneous defender activities, large number of diverse potential threads, and hundreds of potential end-users.
...
In contrast to the three before mentioned security applications, PROTECT relies on a quantal response (QR) \cite{quantal} model of the adversary's behaviour, i.e. the attacker has only bounded rationality.
...
Patrol units move about the system and fine fare-evading passengers.
The optimization objective is to maximize the total revenue (ticket sales plus penalties) to the government.
In \cite{trusts2}, the authors\linebreak extend the TRUSTS system to incorporate also crime prevention and counter-terrorism.
...
Furthermore, the attacker needs some finite time, called \emph{penetration time} (also \emph{attack length}), to successfully complete an attack on a target.
...
As the defender is represented by her resources, we use these two terms interchangeably.
...
The goal of the police is to create a surveillance plan which minimizes the possibility of a successful attack.
...
The results achieved in this thesis can be extended to more general models, which is explained at appropriate places.
...
We assume the play of the game ends after the attack is detected or completed, i.e. no more than one attack is possible.
...
Besides the reasons mentioned in \cite{miso}, the Stackelberg setting together with the game being zero sum lead to one favourable characteristic---the optimal defender's strategy is optimal also with respect to the worst-case scenario.
...
Moreover, to intercept an attack, the defender has to come to the node where the attack is taking place.
...
Authors' subsequent papers concerning multi-robot perimeter patrol in adversarial settings include \cite{perimeter2} and \cite{perimeter3}. In \cite{perimeter2}, it is shown that in the case the adversary has no knowledge of the patrol scheme, a deterministic algorithm is optimal. However, the deterministic algorithm creates high deviations between the \textsf{ppd} values in the nodes and is easy to detect. Therefore, the authors present an algorithm that deals with an adversary who has only partial information. This algorithm maximizes the expected \textsf{ppd} along with minimizing the deviation between the \textsf{ppd} values throughout the perimeter. Its performance was verified by experiments with human subjects.
...
\footnote{The patroller's sensing capabilities in our model are equivalent to the case of $V$ being an identity matrix.}
...
For each possible outcome, patroller's and intruder's payoffs are defined.\footnote{The game is a general-sum game with certain consistency constraints over the players' payoffs.} Explicitly, these values are known to both players. However, the model can capture the case when the intruder's preferences over the targets are uncertain to the patroller.
...
which uses no more than length ${|\overline{H}|}$ of the history, i.e. for any ${|H| > |\overline{H}|}$ the patroller's expected utility remains constant. Throughout the paper, it is claimed that this threshold exists. However, in \cite{iti2}, this claim has been proven \emph{incorrect}. Moreover, in \cite{arbitrary}, they formulate a theorem that ${|\overline{H}| = 0}$ for any fully connected topology. That is also \emph{incorrect}. The proof sketch is done on a complete graph with 3 nodes, each with attack length 2. The authors prove that their algorithm gives the same expected utility when ${|H| = 0}$ and ${|H| = 1}$. Generalization of any kind is omitted. The highest expected utility for ${|H| = 0}$ is ${5/9}$. However, as shown in \cite{iti1}, there is a strategy with ${|H| = 2}$ whose expected utility is ${(\sqrt{5} - 1)/2}$, which is strictly greater than ${5/9}$.
...
The first problem addressed in the paper is finding the smallest number of robots needed to capture any attack with probability greater than zero. For a smaller number of robots, the intruder could successfully attack with probability 1. The authors show that this problem is equivalent to finding the smallest number of maximal cliques such that each target belongs to at least one clique. A simplified idea behind this is as follows.

Let us assume a setting with a graph $G$ and a set of targets $T$. Let $T' \subseteq T$ be a subset of targets and let $d_t$ denote the attack length for a target $t$. If there is a subgraph $G' \subseteq G$
%such that $T'$ is a subset of nodes of $G'$
%such that for each $t \in T'$ it holds that
%$t$ is a node of $G'$
such that for each $t \in T'$ and for each node $v$ from $G'$ there is a path from $v$ to $t$ in $G'$ of length at most $d_t$, then a single robot can patrol all the targets $T'$. In other words, the robot has a strategy such that in each node of $G'$, for every target $t \in T'$ the robot will visit $t$ in at most $d_t$ steps with probability strictly greater than zero.
The goal is to find the smallest set of such subgraphs that would cover all the targets.

The authors build an abstraction $Q$ of the original graph $G$. They formulate an algorithm to find the set of all maximal \emph{labelled cliques} in $Q$,
%these cliques represent the before mentioned subgraphs
and\linebreak a linear program to find the smallest labelled clique cover. The result of the program is the smallest number of robots needed to patrol the environment.
...
\emph{Separated strategies}: each single robot computes its own strategy without considering the strategies or positions of the other robots. This reduces the multi-robot patrolling problem into a set of $|R|$ single-robot patrolling problems, each with $O(n^2)$ probability variables.
...
\emph{Maximal clique assignment}: each robot is assigned a maximal labelled clique (in the abstraction $Q$ of $G$). If the cliques overlap, some targets may be patrolled by multiple robots.
...
The results of the experiment show that, in a game with mobile targets, time-dependent strategies lead to significantly higher utility value. In the experimental results presented in the paper, time-dependent strategies preceded the stationary ones by approximately 50--300\%. However, the computational time was 10--1000 times larger. The authors concluded that time-dependent strategies are reasonable to be used.

\

A noticeably novel game model called MRMT\textsubscript{sg} (multiple Mobile Resources protecting Moving Targets) was introduced in \cite{line}. In MRMT\textsubscript{sg}, the defender's strategy space is discretized and the attacker's strategy space is continuous. A major motivation in the paper is the problem of protecting ferries with fast patrol boats.
...
The defender has $W$ mobile patrollers that can move between $A$ and $B$. All patrollers have a common maximum speed $v_m$ and a protection radius $r_e$. A patroller is \emph{protecting} a target if the distance between them is no more than $r_e$. The attacker chooses a certain time and a certain target to attack. The probability of the attack being successful depends on the positions of the patrollers at that time, i.e. the attack is instant and there is no attack length. This probability is a decreasing function of the number of patrollers protecting the target. An attack on a target can be successful even if there is a patroller protecting the target. The case when an attack on a target is successful iff the target is not protected by any patroller is just a special case.

The authors present an efficient linear program called CASS (Solver for Continuous Attacker Strategies) to solve  MRMT\textsubscript{sg}. The efficiency of CASS consists in the representation of the defender's strategies as marginal probability variables and in using \emph{sub-interval analysis} to model the attacker's continuous strategy space.
...
For clarity, we summarize the basic properties of the previously described patrolling game models in Table \ref{table1}.
We do not include the model from \cite{line} as it deviates from all the other models in a significant way.

\vspace{-3pt}
\begin{table}[h]
\renewcommand\thetable{1}
\hspace{-6pt}
\begin{tabular}{|  c || C{1.225cm} | C{1.83cm} | C{1.45cm} | C{0.865cm} | C{3.22cm} |}
\hline
	& unified attack length	& defender's strategy	& multiple robots	& zero-sum game & special properties \\
\hline \hline
\cite{perimeter}, \cite{perimeter2}, \cite{perimeter3}	& yes	& stochastic	& yes	& yes	& graph is a circle, all nodes are targets \\
\hline
\cite{arbitrary}, \cite{powerlessIntruder}	& no	& finite-memory	& no	& no	& robot has sensing capabilities \\
\hline
\cite{multi-robot} & no	& stochastic	& yes	& yes & targets may have different values \\
\hline
\cite{movingTargets} & yes	& stochastic	& no	& no	& targets move over the nodes in time \\
\hline
this thesis & yes	& infinite-memory	& yes	& yes	& \\
\hline
\end{tabular}
\caption{Comparison of models in related works}
\label{table1}
\end{table}
...
This example is rather sophisticated than the previous two, because the defender cannot protect all the targets with probability 1.
...
We present a modified algorithm for computing an $\epsilon$-optimal defender's strategy.
...
When we realize that $Q$ is the set of all $d$-commitments $f$ occurring in vertices of $\mathbf{Reach}(\theta_\emph{Max})$, it is easy to get the following statement using Lemma \ref{lem:epsilon}.
...
In \cite{iti1}, the authors present an algorithm which computes the desired strategy.
...
To reduce the number of possible states of the automaton $\mathcal{A}$ encoding an $\epsilon$-optimal strategy, we define an equivalence relation on the states of $\mathcal{A}$ which we use to factorize the state-space of $\mathcal{A}$.
...
Given states $p, q \in Q$, $q \neq q_0$, we denote by $\mathcal{A}_{p \leftarrow q} = (Q', U, \delta', q_0, \emptyset)$ the DFA defined as follows:
\vspace{-5pt}
\begin{itemize}
...
Given $p, q \in Q$, $q \neq q_0$, such that $\mathbf{Char}_p = \mathbf{Char}_q$, we have that for all $r \in Q \smallsetminus \{q\}$
...
\begin{definition}
For every characteristic $c$, we define the value of $c$ as
$$val(c) = \min_{t \in T} c(d, t)~.$$
%Given a state $p \in Q$, we define the value of its characteristic by $$val(\mathbf{Char}_p) = \min_{t \in T} \mathbf{Char}_p(d, t)~.$$
\end{definition}
Note that when the play is in a state $p$, the probability of detecting an immediate attack is $\mathbf{Char}_p$.
We have that $att\text{-}val(\sigma_{red} \mid h) = val(\mathbf{Char}_p)$ where $p = \mathcal{A}[h^+]$.
...
%TODONE vypustit historiu $h$, ak ju nepotrebujem
...
The algorithm we present to compute the desired $\epsilon$-optimal strategy is a modification of the algorithm from \cite{iti1}, but instead of searching the set of discretized $d$-commitments, we search the set of discretized characteristics.
...
%The problem of how to determine whether a characteristic is representable will be addressed later.
...
We say that the sequence $(c_1, \dots, c_n)$ are \emph{successors} of $c$ if 
...
We say that a non-empty set $B \subseteq \mathit{Char}^s$ is \emph{closed} if for every $c \in B$ there are successors $(c_1, \dots, c_n) \in B^n$ of $c$.
Finally, we say that a set $D \subseteq \mathit{Char}^s$ is \emph{complete} if it contains a closed subset.

Note that each closed set $B$ represents a set of states of an automaton whose transition function can be built from the states' successors.\footnote{In general, there might be several different sequences of successors for a characteristic in a closed set. Therefore, the process of building an automaton from a closed set is not deterministic.}
And for the automaton's initial state it suffices to choose some $\hat{c} \in B$ such that $\hat{c}(0, \hat{u}) = 1$.\footnote{There has to be at least one such $\hat{c} \in B$ unless the value of all characteristics from $B$ is 0.}

Let us fix a closed set $B$ and for each $c \in B$, let us fix some successors $(c_1, \dots, c_n) \in B^n$ of $c$.
Further, let us fix some $\hat{c} \in B$ such that $\hat{c}(0, \hat{u}) = 1$.
We construct a DFA $\mathcal{A}_B = (B, U, \delta_B, \hat{c}, \emptyset)$ where $\delta_B(c, u_i) = c_i$ for all $c \in B$ and $u_i \in U$.
We define the strategy determined by $\mathcal{A}_B$ as $\sigma_B(h)(u) = \mathcal{A}[h^+](1, u)$.
We show that, for such a strategy, each state of $\mathcal{A}_B$ is identical with its own characteristic.

\begin{lemma}
\label{lem:char=char}
For all states $c \in B$ we have that $c = \mathbf{Char}_c$.
\end{lemma}

\begin{proof}
We prove by induction on $k$ that for all $c \in B$, $u \in U$, and $0 \leq k \leq d$ it holds that $c(k, u) = \mathbf{Char}_c(k, u)$.
...
c(k, u)
  ...
  &= c(1, u) + \sum_{i = 1, u_i \neq u}^n c(1, u_i) \cdot \mathbf{Char}_{\delta_B(c, u_i)}(k - 1, u)\nonumber \\
  &= c(1, u) + \sum_{v \in U \smallsetminus \{u\}} c(1, v) \cdot \mathbf{Char}_{\delta_B(c, v)}(k - 1, u)\nonumber \\
...
By applying Lemma \ref{lem:char=char}, it follows that $val(\sigma_B) \geq \min_{c \in B} val(c)$.
...
That means that in order to find an $\epsilon$-optimal strategy, it suffices to compute a closed set $B$ which maximizes $\min_{c \in B} val(c)$.
...
Following the procedure we have described, this can take up to $|D|\cdot|D|^{(n+1)}$ steps---\linebreak
...
For $|\mathit{Char}^s| = (1/s)^{d^2\cdot{n}}$, $s = \lceil(|U|d)/\epsilon\rceil^{-1}$, and $n = |U|$ we get that the Algorithm \ref{alg:algorithm1} runs in time $\left(\frac{d\cdot|U|}{\epsilon}\right)^{\mathcal{O}(d^2\cdot|U|^2)}$, which proves Theorem \ref{thm:complexity}.
...
And computing a $(2|U|)^{-1}$-optimal strategy is at least that hard.
\end{remark}
...
With some unavoidable modifications for the multi-robot setting, we follow the process from Chapters \ref{chap:existence} and \ref{chap:main}.
As we often refer to some parts from these chapters, it is a huge advantage for the reader to be familiar with them.
At the end of this chapter, we present an algorithm for computing a regular $\epsilon$-optimal strategy which is doubly exponential in the number of robots.
...
We chose to study the least restrictive model, which corresponds to \emph{joint strategy} (one strategy regarding the movement of all the robots is computed centrally) with \emph{full assignment} (each robot can potentially visit every node).
...
\begin{definition}
A \emph{multi-robot patrolling game (MRPG)} is a tuple $\mathcal{G} = (U, E, T, d, z)$ where $U$ is a finite set of \emph{nodes}, $E \subseteq U \times U$ is a set of \emph{edges}, $T \subseteq U$ is a set of \emph{target nodes}, $d$ is the \emph{attack length}, and $z$ is the number of \emph{robots}.
\end{definition}
\noindent
We consider all robots to be homogeneous and we denote them as $r_1, \dots, r_z$.
In every moment of a play, every robot is located in some node.
We define a \emph{configuration} to be a $z$-tuple of nodes specifying the positions of the robots~\cite{multi-robot}.
We denote it by $c = (c_1, \dots, c_z)$, where $c_i \in U$ is the position of the robot $r_i$.
We denote by $C = U^z$ the set of all configurations.

Note that, as opposed to a single-robot patrolling game (SRPG), an MRPG does not have any predefined initial configuration of the robots.
...
For each configuration $c \in C$, we define $\mathit{cover}(c) = \{c_1, \dots, c_z\}$ to be the \emph{cover} of $c$, representing all the nodes covered by the configuration.
A cover can contain from $1$ to $z$ nodes.
For each node $u \in U$, we define $C_u = \{c \in C \mid u \in \mathit{cover}(c)\}$ to be the set of all configurations that cover $u$.

Given a configuration $c \in C$, we define $succ(c) := \{c' \in C \mid c'_i \in succ(c_i)~\text{for all}~1 \leq i \leq z\}$ to be the set of all \emph{successors} of $c$.
...
\section{Existence of Optimal Strategies}
...
Let us fix a multi-robot patrolling game $\mathcal{G} = (U, E, T, d, z)$.
...
We claim that an extra condition\footnote{The extra condition listed in \cite{iti1} we refer to is that for every $v \in V$, the set $\mathbf{E}(v)$ is compact (i.e. bounded and closed) and a function $val^{\mathcal{S}}[\cdot](v, t)$ which to a given $a \in \mathbf{E}(v)$ assigns $\sum_{v'\in V}\mathbf{P}(v, a)(v')\cdot val^{\mathcal{S}}(v', t)$ is continuous on $\mathbf{E}(v)$.} sufficient for a stochastic game to have an optimal memoryless strategy for player \emph{Max} is satisfied by $\mathcal{S}$ by the same reason as it is satisfied by the stochastic game $\mathcal{S}$ defined in Section \ref{sec:translation}.
...
\section{Discretized $\epsilon$-optimal Strategies}
\label{sec:discretizationMRPG}

In this section, we prove that each optimal defender's strategy $\sigma$ retains its value throughout a play of the game.
We later use this knowledge to prove that an appropriately discretized strategy (for a given precision $\epsilon$) has value at least $val(\sigma) - \epsilon$ in every moment of the play, and therefore is $\epsilon$-optimal.
We translate this discretized strategy to a strategy of player \emph{Max} in a stochastic game, from which we build a DFA encoding a regular $\epsilon$-optimal strategy.

As a defender's strategy in an MRPG needs to be defined also for the empty word, we start with a modified definition of a defender's strategy after a history has been played.
...
Also note that for $c$ being the $\sigma$-initial configuration, we have $\sigma_{c} = \sigma$.
That is because the attacker is allowed to attack no sooner than in the initial configuration.
...
A higher precision (smaller $s$) is needed because, for an optimal strategy $\sigma$ and a history $h$, the support of $\sigma(h)$ may be of size up to the number of all configurations.
Then, we have to prove that the resulting discretized strategy $\sigma_\epsilon$ is $\epsilon$-optimal.
...
To formalize this, we start by defining the value of a defender's strategy against an immediate attack after a given relevant history.
...
\end{lemma}
\begin{proof}
We proceed in the same way as in the proof of Lemma \ref{lem:epsilon}, so we will be concise in reasoning.
We prove that
\begin{align}
att\text{-}val(\sigma_\epsilon \mid h)
...
Let us denote the difference $R[\sigma, h, t, k] - R[\sigma_\epsilon, h, t, k]$ by $\Delta$.
For $k \geq 2$, we have \pagebreak%TODO
\begin{eqnarray}
\hspace{-3.3pt}
\Delta
...
For more details, see Step 2 from Section \ref{sec:discretization}.\pagebreak%TODO
...
For every history $h \in \mathcal{H}_\epsilon$, we define:
...
Notice that the behaviour of the strategy in the first two steps (for histories of length $0$ and $1$) is encoded by the initial state $q_0$.
To determine what to play in histories $h$ of length at least $2$, the correct state has to be computed based on the transition function $\delta$ and the input $h^+$.
...
To reduce the state-space of the automaton encoding an $\epsilon$-optimal strategy, we again define an equivalence relation on its states based on the ability to defend an attack which started somewhen in the last $d$ steps.
However, in contrast to SRPGs where this information alone was enough to define the equivalence relation, in MRPGs we have to take into account some additional properties of a state.

In SRPGs, a characteristic contained all the information needed to determine the current location of the defender and the probability distribution over the nodes in the next step.
In MRPGs, an analogous definition of a characteristic of a state (probability of reaching a certain node in the next $k$ steps) would not suffice---probabilities of visiting each particular node in the next step do not contain enough information to determine the probability distribution over the configurations in the next step.
Also, the current configuration cannot be determined by its cover.
Therefore, we extend the definition of a characteristic to contain this additional information---the current configuration and the probability distribution over configurations in the next step.
\mbox{Note that these two properties can be represented by a $1$-commitment.}

We start by defining a characteristic as a general mathematical object.
Then, we map essential properties of a state to such an object.

\begin{definition}
A \emph{characteristic} is a tuple $(f, m)$ where $f$ is a $1$-commitment and $m : \{1, \dots, d\} \times U \to [0, 1]$ is a function satisfying $m(1, u) = \sum_{c \in C_u} \xi(f)(c)$ for all $u \in U$.
%\vspace{-5pt}
%\begin{itemize}
...
\end{definition}
\noindent
We consider two characteristics $(f, m)$ and $(f', m')$ to be equal if $m = m'$, $r(f) = r(f')$ and $\xi(f) = \xi(f')$.

\begin{definition}
Let $\mathcal{A} = (Q, C, \delta, q_0, \emptyset)$ be an automaton encoding a defender's strategy.
Given $p \in Q$, we define the characteristic $\mathbf{Char}_p = (f_p, m_p)$ of the state $p$ to be a characteristic such that, whenever the strategy is in the state $p$, $f_p(\epsilon)$ and $\xi(f_p)$ represent the current configuration and the probability distribution over $C$ in the next step, respectively,
and $m_p$ assigns to every pair $(k, u) \in \{1, \dots, d\} \times U$ the probability of visiting $u$ in at most $k$ steps.
\end{definition}
\noindent
Consider the automaton $\mathcal{A}$ constructed in Step~3 in the previous section and let $\mathbf{Char}_p$ be the characteristic of one of its states $p$.
Observe that
\vspace{-5pt}
\begin{itemize}
	\item $f_p(\epsilon) = p(\epsilon)$,
	\item $\xi(f_p) = \xi(p)$,
	\item and for $2 \leq k \leq d$
$$m_p(k, u) = \sum_{c \in C_u}\xi(f_p)(c) + \sum_{c \in C \smallsetminus C_u}\xi(f_p)(c)\cdot m_{\delta(p, c)}(k-1, u)~.$$
\end{itemize}
\vspace{-5pt}
We will use these observations to prove that two states with the same characteristics can be merged together without influencing the value of the strategy determined by the automaton.
Before we do that, we remember the concept of merging two states of an automaton.
Given states $p, q \in Q$, $q \neq q_0$, we denote by $\mathcal{A}_{p \leftarrow q} = (Q', C, \delta', q_0, \emptyset)$ the DFA defined as follows:
\vspace{-5pt}
\begin{itemize}
	\item $Q' = Q \smallsetminus \{q\}$
	\item $\delta'(r, c) =
  \begin{cases}
   p		& \text{if } \delta(r, c) = q \\
   \delta(r, c)	& \text{otherwise.}
  \end{cases}$
\end{itemize}
\vspace{-5pt}

The following lemma states that if a state is substituted by another state with the same characteristic, the characteristics of the other states remain unchanged.
It is a copy of Lemma \ref{lem:characteristics}, yet with a modified proof.

\begin{lemma}
\label{lem:characteristicsMRPG}
Given $p, q \in Q$, $q \neq q_0$, such that $\mathbf{Char}_p = \mathbf{Char}_q$, we have that for all $r \in Q \smallsetminus \{q\}$
$$\mathbf{Char}'_r = \mathbf{Char}_r$$
where $\mathbf{Char}'_r$ denotes the characteristic of $r$ in the automaton $\mathcal{A}_{p \leftarrow q}$.
\end{lemma}
...
\begin{proof}
As a state completely determines the $1$-commitment of its characteristic, we have $f_r = f'_r$ for every state $r$.
Further, we prove by induction on $k$ that for all $r \in Q \smallsetminus \{q\}$, $u \in U$, $1 \leq k \leq d$ we have that $$m'_r(k, u) = m_r(k, u)~.$$
For $k = 1$, this clearly holds because for any characteristic $(f, m)$, values $m(1, u)$ are completely determined by $\xi(f)$, and $f_r = f'_r$.

For $2 \leq k \leq d$, it follows from the definition of the characteristic of a state, from the induction hypothesis and from the definition of $\delta'$ that
\begin{align}
m'_r(k, u)
  &= \sum_{c \in C_u}\xi(f'_r)(c) + \sum_{c \in C \smallsetminus C_u}\xi(f'_r)(c)\cdot m'_{\delta'(r, c)}(k-1, u)	\nonumber \\
  &= \sum_{c \in C_u}\xi(f'_r)(c) + \sum_{c \in C \smallsetminus C_u}\xi(f'_r)(c)\cdot m_{\delta'(r, c)}(k-1, u)	\nonumber \\
  &= \sum_{c \in C_u}\xi(f'_r)(c) + \sum_{c \in C \smallsetminus C_u}\xi(f'_r)(c)\cdot m_{\delta(r, c)}(k-1, u)	\nonumber \\
  &= m_r(k, u)~.\nonumber
\end{align}
\end{proof}
\noindent
Note that Lemma \ref{lem:characteristicsMRPG} holds also for the reduced automaton $\mathcal{A}_{p \leftarrow q}$ and the process of merging equivalent states can be repeated until all states have mutually different, but still unchanged characteristics.
As this process is non-deterministic, let us fix and denote by $\mathcal{A}_{red}$ the resulting automaton which cannot be further reduced.
We denote by $Q_{red}$ and $\sigma_{red}$ the set of states of $\mathcal{A}_{red}$ and the defender's strategy determined by $\mathcal{A}_{red}$, respectively.

We show that $\sigma_{red}$ is $\epsilon$-optimal by arguing that whenever the play is in a state $p$, the probability of capturing an immediate attack is at least $val(\sigma) - \epsilon$.
We start by defining this probability as a value of the characteristic of a state.

\begin{definition}
For every characteristic $(f, m)$, we define its value as
$$val((f, m)) = \min_{t \in T} m(d, t)~.$$
\end{definition}
Note that $att\text{-}val(\sigma_{red} \mid h) = val(\mathbf{Char}_p)$ where $p = \mathcal{A}[h^+]$.
However, it follows from the definition of the automaton $\mathcal{A}$ and the induced strategy $\sigma_\epsilon^{reg}$ that $val(\mathbf{Char}_p) = val(p)$ for each state $p \in Q$.
By applying Lemma~\ref{lem:epsilon_commitmentMRPG}, we get the following:

\begin{lemma}
\label{lem:char_valueMRPG}
For each state $p \in Q$ we have $val(\mathbf{Char}_p) \geq val(\sigma) - \epsilon$.
\end{lemma}
\noindent
As $Q_{red} \subseteq Q$ and the characteristic of any state in $\mathcal{A}_{red}$ is the same as in $\mathcal{A}$, an immediate consequence of Lemma \ref{lem:char_valueMRPG} is that $val(\sigma_{red}) \geq val(\sigma) - \epsilon$, i.e. $\sigma_{red}$ is $\epsilon$-optimal.

Finally, we identify each state $p \in Q_{red}$ with its characteristic $\mathbf{Char}_p = (f_p, m_p)$ to obtain an automaton $\mathcal{A}_\mathit{Char} = (Q_\mathit{Char}, C, \delta_\mathit{Char}, \mathbf{Char}_{q_0}, \emptyset)$ where
\vspace{-20pt}
\begin{itemize}
	\item $Q_\mathit{Char} = \{\mathbf{Char}_p \mid p \in Q_{red}\}$,
	\item $\delta_\mathit{Char}(\mathbf{Char}_p)(c) = \mathbf{Char}_q$ iff $\delta_{red}(p)(c) = q$.
\end{itemize}
\vspace{-5pt}
We use the $1$-commitment part of a characteristic to define the defender's strategy.
We define $\sigma_\mathit{Char}(\epsilon) = f_{q_0}(\epsilon)$ and, for every history $h \in \mathcal{H}$, we define
$\sigma_\mathit{Char}(h) = \xi(f)$ where $(f, m) = \mathcal{A}_\mathit{Char}[h^+]$.
%For every history $h \in \mathcal{H}_\epsilon$, we define:
%$$\sigma_\mathit{Char}(h) =
%   \begin{cases}
%   f_{q_0}(\epsilon)		& \text{if } h = \epsilon \\
%   \xi(f)			& \text{otherwise, where } (f, m) = \mathcal{A}_\mathit{Char}[h^+].
%   \end{cases}$$
Because for each $d$-commitment $p \in Q$ it holds that $f_p(\epsilon) = p(\epsilon)$ and $\xi(f_p) = \xi(p)$, we have that $\sigma_\mathit{Char} = \sigma_{red}$, and therefore $\sigma_\mathit{Char}$ is $\epsilon$-optimal.

\section{The Algorithm}

In this section, we present an algorithm to compute a regular $\epsilon$-optimal strategy of a given MRPG for a given precision $\epsilon$.
This algorithm is almost the same as the Algorithm \ref{alg:algorithm1} in Section \ref{sec:alg}.
And although the ideas behind these algorithms are very similar, the argumentation about their correctness differs due to different definitions of a characteristic in SRPGs and MRPGs.
Therefore, we provide a detailed argumentation about the algorithm.

From the previous section, we already know that there exists a DFA $\mathcal{A}_\mathit{Char}$ which encodes an $\epsilon$-optimal strategy and whose states $Q_\mathit{Char}$ are mutually different characteristics.
Further, each of its states $\gamma = (f, m)$ satisfies:
\vspace{-20pt}
\begin{itemize}
	\item $\gamma = \mathbf{Char}_\gamma$
	\item $\xi(f)(c)$ is an integer multiple of $s$ for all $c \in C$, where $s = \lceil(|C|d)/\epsilon\rceil^{-1}$
	\item $val(\gamma) \geq val(\sigma) - \epsilon$.
\end{itemize}
\vspace{-5pt}
\noindent
We aim to find an automaton satisfying all of these conditions.
We consider the set of all \emph{discretized} characteristics
$\mathit{Char}^s$ that consists of all characteristics $(f, m)$ such that $\xi(f)(c)$ is an integer multiple of $s$ for all $c \in C$ and $m(k, u)$ is an integer multiple of $s^k$ for all $1 \leq k \leq d$ and $u \in U$.
Realizing that values $\sigma_\mathit{Char}(h)(c)$ are integer multiples of $s$, it is easy to see that $Q_\mathit{Char} \subseteq \mathit{Char}^s$.
...
\begin{algorithmic}[1]
\par\vspace{0.5mm}
	\Require	multi-robot patrolling game $\mathcal{G} = (U, E, T, d, z)$, $\epsilon > 0$
	\Ensure	a regular $\epsilon$-optimal strategy $\sigma_\epsilon^{alg}$
\par\vspace{3.5mm}

\State $\Gamma \leftarrow$ empty priority queue; $D \leftarrow \emptyset$
\State \textbf{for each} $\gamma \in \mathit{Char}^s$ \textbf{do}
		add $\gamma$ to $\Gamma$ with priority $val(\gamma)$
\vspace{3px}
\Repeat
	\State pull from $\Gamma$ the item $\gamma$ with the highest priority
	\State $D \leftarrow D \cup \{\gamma\}$
\Until {$D$ is \emph{complete}}
\vspace{3px}
\State $\sigma_\epsilon^{alg} \leftarrow$ the strategy induced by a closed subset of $D$
\end{algorithmic}
...
Let $C = \{c_1, \dots, c_{|C|}\}$ be the set of all configurations
and let $\gamma, \gamma_1, \dots, \gamma_{|C|}$ be some characteristics (not necessarily different) such that $\gamma = (f, m)$ and $\gamma_i = (f_i, m_i)$.
We say that the sequence $(\gamma_1, \dots, \gamma_{|C|})$ are \emph{successors} of $\gamma$ if
the following two conditions are satisfied:
\vspace{-5pt}
\begin{itemize}
%\begin{enumerate}[label=(\Alph*)]
	\item if $\xi(f)(c_i) > 0$, then $r(f_i) = c_i$,%\label{conditionA}
	\item for all $2 \leq k \leq d$ and $u \in U$
$$m(k, u) = \sum_{c \in C_u}\xi(f)(c) + \sum_{i = 1, c_i \notin C_u}^{|C|} \xi(f)(c_i)\cdot m_i(k-1, u)~.$$
\end{itemize}
%\end{enumerate}
\vspace{-5pt}
\noindent
The meaning of this is that if, for all $i$, $\gamma_i = \mathbf{Char}_{\gamma_i}$ and there is a transition from the state $\gamma$ to $\gamma_i$ under $c_i$, then also $\gamma = \mathbf{Char}_{\gamma}$.
We refer to $\gamma_i$ as the $c_i$-successor of $\gamma$.

\begin{remark}
The first condition guarantees that if there is a positive probability of choosing $c_i$ in the state $\gamma$, then the current configuration of its $c_i$-successor must be $c_i$.
The condition's if-clause is crucial for the algorithm---there might exist a configuration $c$ and a target $t$ such that no successor of $c$ covers $t$.
Therefore, if $d = 1$, the value of any characteristic $(f, m)$ such that $r(f) = c$ would be $0$ and no characteristic could have a $c$-successor from the set $D$ until characteristics with value $0$ were added to $D$.
Thus, the $c$-successor of a characteristic $(f, m)$ can be an arbitrary characteristic as long as $\xi(f)(c) = 0$.
\end{remark}

As in Section \ref{sec:alg}, we say that a non-empty set $B \subseteq \mathit{Char}^s$ is \emph{closed} if for every $\gamma \in B$ there are successors $(\gamma_1, \dots, \gamma_{|C|}) \in B^{|C|}$ of $\gamma$.
Finally, we say that a set $D \subseteq \mathit{Char}^s$ is \emph{complete} if it contains a closed subset.
Observe that each closed set can be seen as the set of states of an automaton whose transition function can be build from the states' successors.
Contrary to SRPGs, the initial state of this automaton can be any of its states as an MRPG does not force a specific initial configuration.

Let us fix a closed set $B$ and an arbitrary characteristic $\hat{\gamma} \in B$, $\hat{\gamma} = (\hat{f}, \hat{m})$.
Further, for each $\gamma \in B$, let us fix some successors $(\gamma_1, \dots, \gamma_{|C|}) \in B^{|C|}$ of $\gamma$.
We construct a DFA $\mathcal{A}_B = (B, C, \delta_B, \hat{\gamma}, \emptyset)$ where $\delta_B(\gamma, c_i) = \gamma_i$ for all $\gamma \in B$ and $c_i \in C$.
We define a defender's strategy $\sigma_B$ determined by $\mathcal{A}_B$ as
$\sigma_B(\epsilon) = \hat{f}(\epsilon)$ and, for every history $h \in \mathcal{H}$, we define
$\sigma_B(h) = \xi(f)$ where $(f, m) = \mathcal{A}_B[h^+]$.
We show that each state of $\mathcal{A}_B$ is identical with its own characteristic.

\begin{lemma}
\label{lem:char=charMRPG}
For every state $\gamma \in B$ we have that $\gamma = \mathbf{Char}_\gamma$.
\end{lemma}

\begin{proof}
To be clear on the notation, let $\gamma = (f, m)$ and $\mathbf{Char}_\gamma = (f_\gamma, m_\gamma)$.
From the definition of $\sigma_B$, it directly follows that $f = f_\gamma$.

For the second part, we prove by induction on $k$ that for all $\gamma \in B$, $u \in U$, and $1 \leq k \leq d$ it holds that $m(k, u) = m_\gamma(k, u)$.
For $k = 1$, this follows from the definition of the characteristic and the fact that $f = f_\gamma$.

For $2 \leq k \leq d$, let $(\gamma_1, \dots, \gamma_{|C|})$ be the successors of $\gamma$. It follows that
\begin{align}
m(k, u)
  &= \sum_{c \in C_u}\xi(f)(c) + \sum_{i = 1, c_i \notin C_u}^{|C|} \xi(f)(c_i)\cdot m_i(k-1, u)	\label{eq6:first} \\
  &= \sum_{c \in C_u}\xi(f_\gamma)(c) + \sum_{i = 1, c_i \notin C_u}^{|C|} \xi(f_\gamma)(c_i)\cdot m_{\gamma_i}(k-1, u)	\label{eq6:second} \\
  &= \sum_{c \in C_u}\xi(f_\gamma)(c) + \sum_{i = 1, c_i \notin C_u}^{|C|} \xi(f_\gamma)(c_i)\cdot m_{\delta_B(\gamma, c_i)}(k-1, u)	\label{eq6:third} \\
  &= \sum_{c \in C_u}\xi(f_\gamma)(c) + \sum_{c \in C \smallsetminus C_u} \xi(f_\gamma)(c)\cdot m_{\delta_B(\gamma, c)}(k-1, u)	\label{eq6:fourth} \\
  &= m_\gamma(k, u)\label{eq6:fifth}~.
\end{align}
The equality \eqref{eq6:second} follows from the fact that $f = f_\gamma$ and from the induction hypothesis (we used notation $\mathbf{Char}_{\gamma_i} = (f_{\gamma_i}, m_{\gamma_i})$).
The other equalities follow from the definitions of successors (\eqref{eq6:first}), of $\sigma_B$ (\eqref{eq6:third}), of $C$ (\eqref{eq6:fourth}) and of the characteristic of a state (\eqref{eq6:fifth}).
\end{proof}

As the lowest value from among the values of characteristics of the automaton's states provides a lower bound on the value of the defender's strategy determined by the automaton, it follows from Lemma \ref{lem:char=charMRPG} that
$val(\sigma_B) \geq \min_{\gamma \in B} val(\gamma)$.

However, it follows from the existence of the automaton $\mathcal{A}_\mathit{Char}$ that there exists a closed set $Q_\mathit{Char}$ such that $\min_{\gamma \in Q_\mathit{Char}} val(\gamma) \geq val(\sigma) - \epsilon$.
And as any closed subset $B$ of the complete set $D$ found by the Algorithm \ref{alg:algorithm2} maximizes $\min_{\gamma \in B} val(\gamma)$, we have that $\sigma_\epsilon^{alg} \geq val(\sigma_\mathit{Char})$.
Therefore, $\sigma_\epsilon^{alg}$ is $\epsilon$-optimal.

The process of determining whether a finite set $D$ of characteristics is complete is the same as in Section \ref{sec:alg}.
For a copy $D'$ of $D$, we iteratively remove from $D'$ all the characteristics which do not have successors in $D'$ until we reach a fixed point, in which all characteristics have successors.
If the fixed point is not empty, then it is the desired closed set and $D$ is complete.

\

\noindent
\textbf{Complexity of the Algorithm \ref{alg:algorithm2}}.
Just as for the Algorithm \ref{alg:algorithm1}, the decisive part of the execution time is spent in determining whether a set $D$ is complete.
This can take up to $|D|\cdot|D|^{|C|+1}$ steps---$|D|$ iterations of verifying for each tuple $(\gamma_0, \dots, \gamma_{|C|}) \in D'^{|C|+1}$ whether $(\gamma_1, \dots, \gamma_{|C|})$ are successors of $\gamma_0$.
As $D \subseteq \mathit{Char}^s$, it remains to find an upper bound for $|\mathit{Char}^s|$, i.e. to determine how many different discretized characteristics $(f, m)$ exist.
The number of possible values for $r(f)$ is $|C|$, for $\xi(f)$ it is $(s^{-1})^{|C|}$, and for $m$ it is $(s^{-1})^{d^2|U|}$.
For $C = U^z$ and $s = \lceil(|C|d)/\epsilon\rceil^{-1}$ we get that $|\emph{Char}^s| \leq \left(\frac{d\cdot|U|^z}{\epsilon}\right)^{|U|^{z} + d^2\cdot|U|}$.\footnote{To achieve a slightly lower bound, we used the knowledge that the values $m(1, u)$ are completely determined by $f$.}\linebreak
By exponentiating this upper bound to $|C| + 1$, we get the following result:

\begin{thm}
\label{thm:complexityMRPG}
Let $\epsilon > 0$.
There is a regular $\epsilon$-optimal defender's strategy computable in time
$\left(\frac{d\cdot|U|^z}{\epsilon}\right)^{\mathcal{O}(|U|^{2z} + d^2\cdot|U|^{z+1})}$.
\end{thm}

\begin{remark}
As all the robots are homogenous, it would be possible to define the multi-robot model in such a way that multisets of nodes are considered instead of configurations.
However, even though the number of possible multisets ${|U| + z - 1 \choose z}$ is strictly smaller than $|C|$, it would not make any significant improvement to the worst-case asymptotic complexity.
\end{remark}

\chapter{Conclusions and Future Work}

The purpose of this thesis was to extend the previous results for patrolling games (to present a more efficient algorithm to solve instances of such games) as well as to define a more general model and apply the achieved results to it.

First, we looked into the simple model of patrolling games with one defender and one attacker.
We built on past results such as the existence of an $\epsilon$-optimal defender's strategy and a doubly exponential algorithm to compute such a strategy.
We reformulated some statements with more detailed proofs.
That allowed us to utilize the concept of characteristics to factorize the state space of the automaton encoding an $\epsilon$-optimal strategy.
We presented an algorithm for computing an $\epsilon$-optimal strategy in only quadratically exponential time.

Second, we generalized the model of patrolling games by equipping the defender with multiple homogeneous robots.
We studied the least restrictive model where a single defender's strategy determines the movement of all robots and each robot can potentially traverse the whole game graph.
We adapted the results for single-robot patrolling games to multi-robot patrolling games and presented an algorithm to compute an $\epsilon$-optimal strategy for any MRPG.
The resulting automaton encoding the strategy is doubly exponential in the number of robots.
%To the best of our knowledge, this is the first algorithm which can an $\epsilon$-optimal strategy for MRPG among all history-dependent strategies.

There is a lot of space for future research in this area.
Generalization of our model is possible in many different directions.
Target-specific attack length has been already addressed in \cite{iti2}.
Introducing sensing capabilities for robots should be straightforward---only a simple change in the definition of a defended run should be needed.
A more challenging generalization of the model would be multiple attackers.
Also, a multi-robot model with heterogeneous robots or more restrictive environment could be considered.

As long as it takes improving the computational complexity, we see opportunities in reducing the state space of the automaton.
This might be achieved by a different equivalence relation.
For example, we still do not know whether, in general, the automaton's state-space can be bounded above by the number of different $1$-commitments, i.e. no two states would have the same probability distribution in their root. However, we believe that this cannot be done.
Inspecting the properties of optimal strategies might reveal new ways for more efficient algorithms.
Finally, applying some advanced results from other fields of mathematics might significantly reduce the time needed to determine whether a set of characteristics is complete.
...
\bibitem{miso} M. Abaffy. Patrolling Games on Graphs [online]. 2013 [cit. 2014-05-10].
%Diploma thesis. Masaryk University, Faculty of Informatics.
Available at $<$http://is.muni.cz/th/321758/fi\texttt{\us}m/$>$.

\bibitem{iti1} M. Abaffy, T. Brázdil, V. Øehák, B. Bošanský, A. Kuèera, and J. Krèál. Solving Adversarial Patrolling Games with Bounded Error (Extended Abstract). In \emph{AAMAS}, pages 1617-1618, 2014.
%Online: $<$http://goo.gl/ueKS79$>$

\bibitem{iti2} T. Brázdil, P. Hlinìný, A. Kuèera, and V. Øehák. Optimal Strategies in Adversarial Patrolling Games. Unpublished.
...
\bibitem{gameTheory} Wikipedia. Game Theory [online]. 2014 [cit. 2014-05-23]. Available at
...
\bibitem{protect_coastguard} Teamcore Research Group. PROTECT - US Coast Guard [online]. 2014 [cit. 2014-06-01]. Available at $<$http://teamcore.usc.edu/projects/ coastguard/$>$.
