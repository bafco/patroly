...
We say that a sequence $(c_1, \dots, c_n)$ are \emph{successors} of $c$ if the following two conditions are satisfied
\vspace{-5pt}
\begin{itemize}
	\item $c(k, u) = \sum_{i = 1}^n c(1, u_i) \cdot c_i(k - 1, u)$ for all $1 \leq k \leq d$ and $u \in U$,
	\item if $c(1, u_i) > 0$, then $c_i(0, u_i) = 1$.% for all $1 \leq i \leq n$.
\end{itemize}
\vspace{-5pt}
...
We say that a non-empty set $R \subseteq Char^s$ is \emph{closed} if for all $c \in R$ there are successors $(c_1, \dots, c_n) \in R^n$ of $c$.
...
We construct a DFA $\mathcal{A}_{Char} = (R, U, \delta_{Char}, \hat{c}, \{\})$ where $\delta_{Char}(c, u_i) = c_i$ for all $c \in R$ and $u_i \in U$.
%\vspace{-3pt}
%\begin{itemize}
%	\item $Q_{Char} = R$ %is the finite set of states
%	%\item $U$ is the alphabet
%	\item $\delta_{Char}(c, u_i) = c_i$ where $c_i$ is the fixed $u_i$-successor of $c$
%	\item $q_0 = \hat{c}$.
%\end{itemize}
%\vspace{-3pt}
We define the strategy determined by $\mathcal{A}_{Char}$ as $\sigma_{Char}(h)(u) = \mathcal{A}[h^+](1, u)$.
%$\sigma_{Char}(h) = \psi_{\mathcal{A}[h^+]}$ (recall that $\psi_c(u) = c(1, u)$ for a characteristic $c$).
We show that for such a strategy, each state of $\mathcal{A}_{Char}$ is identical with its own characteristic.

%TODO \newpage ?
\begin{lemma}
\label{lem:char=char}
For all $c \in R$ we have that $c = \mathbf{Char}_c$.
\end{lemma}

\begin{proof}
We prove by induction on $k$ that for all $c \in R$ and $u \in U$ it holds that $c(k, u) = \mathbf{Char}_c(k, u)$.
Let $(c_1, \dots, c_n)$ be the successors of $c$.
...
\begin{align*}
c(k, u)
  &= \sum_{i = 1}^n c(1, u_i) \cdot c_i(k - 1, u) \\ %from the definition of successors
  &= \sum_{i = 1}^n c(1, u_i) \cdot \mathbf{Char}_{c_i}(k - 1, u) \\ %from induction hypothesis
  &= \sum_{i = 1}^n c(1, u_i) \cdot \mathbf{Char}_{\delta_{Char}(c, u_i)}(k - 1, u) \\ %%%
  &= \sum_{v \in U} c(1, v) \cdot \mathbf{Char}_{\delta_{Char}(c, v)}(k - 1, u) \\ %$U = \{u_1, \dots, u_n\}$
  &= c(1, u) + \sum_{v \neq u} c(1, v) \cdot \mathbf{Char}_{\delta_{Char}(c, v)}(k - 1, u) \\ % from the definition of characteristic of a state
%TODO toto mi nejak nesedi
%  &= \psi_c(u) + \sum_{v \neq u}\psi_c(v) \cdot \mathbf{Char}_{\delta_{Char}(c, v)}(k - 1, u) \\ % from the definition of successors we know that ... %TODO
  &= \mathbf{Char}_c(k, u)~.
\end{align*}
...
It suffices to prove that for all $c \in Q$ we have that $val(c) \geq val(\sigma) - \epsilon$.
