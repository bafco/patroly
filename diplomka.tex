...
\chapter{TODO uz dosiahnute vysledky - (Existence of) Optimal Strategies (in Patrolling Games)}

%TODO - really this is what we are goind to do?
In this chapter, we
%describe the process, presented in \cite{iti1}, of translating patrolling games into stochastic games.
summarize the essential results achieved in \cite{iti1}, while we primarily focus on the constructs on which our results are based.

We start by translating a given patrolling game $\mathcal{G}$ into a turn-based perfect-information stochastic safety game $\mathcal{S}$, where the defender corresponds to the maximizer.
...
Then, we show that each optimal strategy satisfies a certain condition which is crucial for argumentation about $\epsilon$-optimality.
...
A \emph{stochastic game} is a tuple $\mathcal{S} = (V, (V_{\emph{Min}}, V_{\emph{Max}}), \ell, A, \bold{E}, \bold{P})$ where $V$ is a set of vertices, $(V_{\emph{Min}}, V_{\emph{Max}})$ is a partition of $V$, $\ell \in \mathbb{N}$, $A \subseteq \mathbb{R}^\ell$ is a set of actions, $\mathbf{E} : V \to 2^A$ assigns to each vertex a set of \emph{enabled actions}, and $\bold{P} : V \times A \to \Delta(V)$ assigns to each $(v, a) \in V \times A$ a discrete probability distribution over $V$ with finite support.
...
When the play is in a vertex $v \in V_\bigcirc$ and player $\bigcirc$ selects an action $a \in \bold{E}(v)$, the next node is chosen randomly according to the distribution $\bold{P}(v, a)$.
%Note that
Players can choose actions based on all the vertices already visited, not just the current one.
However, the players cannot randomize.\footnote{As we will see, for our purposes of translation between patrolling and stochastic games, players in stochastic games need not to randomize---for every vertex $v \in V_{Max}$, the set of enabled actions $\bold{E}(v)$ will be closed under convex combination, and the attacker cannot randomize even in patrolling games.}
Formally, a \emph{strategy} for player $\bigcirc \in \{\emph{Min}, \emph{Max}\}$ is a function $\theta_\bigcirc : V^*V_\bigcirc \to A$ such that $\theta_\bigcirc (wv) \in \bold{E}(v)$ for all $w \in V^*$ and $v \in V_\bigcirc$.

Similarly to patrolling games, every initial vertex $v \in V$ and every pair of players' strategies $(\theta_{Min}, \theta_{Max})$ determine a unique probability space over all infinite sequences of vertices starting in $v$ (see \cite{chung}).
Given an initial vertex $v \in V$, a target vertex $t \in V$, and a pair of players' strategies $(\theta_{Min}, \theta_{Max})$, we denote by $val^\mathcal{S}(\theta_{Min}, \theta_{Max}, v, t)$ the probability of all infinite sequences of vertices starting in $v$ that \emph{never} visit $t$.
We define $val^\mathcal{S}(\theta_{Max}, v, t) = \text{inf}_{\theta_{Min}}val^\mathcal{S}(\theta_{Min}, \theta_{Max}, v, t)$. Furthermore, we define $val^\mathcal{S}(v, t) = \text{sup}_{\theta_{Max}}val^\mathcal{S}(\theta_{Max}, v, t)$.
Given a target $t \in V$, a strategy $\theta_{Max}^*$ is called \emph{optimal} if $val^\mathcal{S}(\theta_{Max}^*, v, t) = val^\mathcal{S}(v, t)$ for all $v \in V$.
...
To cope with this setting, we define the stochastic game in a way that the vertices correspond to defender's $d$-step randomized plans, where $d$ is the attack length.
...
When player \emph{Min} chooses the action corresponding to attacker's waiting, the first step of the previously fixed $d$-step plan is performed and player \emph{Max} has to prolong the plan by one step for it to remain a $d$-step plan.
...
\begin{definition}
Let $k \in \mathbb{N}$. A \emph{$k$-commitment} is a function $f : N^{\leq k} \to \Delta(N)$ satisfying the following conditions:
\vspace{-5pt}
...
A given $d$-commitment $f$ can be interpreted as a Markov chain with $N^{\leq d+1}$ as the set of states and the transition probabilities given by $f$ (the states of $N^{d+1}$ are absorbing).
...
- co sa tyka optimalnej strategie, nevraviet, preco existuje - staci sa odkazat
