...
As the defender's strategy works with configurations instead of single nodes, we need to modify the definition of $k$-commitments.
The notation of terms associated with $k$-commitments stays the same.
Let us fix a patrolling game $\mathcal{G} = (U, E, T, d, R)$.
...
We define a stochastic game $\mathcal{S} = (V, (V_{\emph{Min}}, V_{\emph{Max}}),$ $|C|^{d+2}, A, \mathbf{E}, \mathbf{P})$ in the same way as we did in Section \ref{sec:translation} (taking into account the modified definition of a $d$-commitment) with the only exception that $\mathbf{E}(\mathit{init}) = \mathbf{Comm}_d$, as the initial configuration is strategy-specific.
