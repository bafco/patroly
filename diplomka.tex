...
Using the translation process between patrolling games and stochastic games, we translate the strategy $\sigma_\epsilon$ in $\mathcal{G}$ to a strategy $\theta_\emph{Max}$ in $\mathcal{S}$ such that
$$val^\mathcal{S}(\theta_\emph{Max}, \mathit{init}, \mathit{fail}) = val(\sigma_\epsilon)~.$$
