...
Using Lemma \ref{lem:difference}, we prove by induction on $k$ that
\begin{align}
...
Finally, we prove that the inequality \eqref{eq1:induction} holds for all $h \in \mathcal{H}$, $t \in T$, and $k \in \mathbb{N}$.
The base case ($k = 1$) directly follows from Lemma \ref{lem:difference} and the fact that $R[\iota, h, u, 1] = \iota(h)(u)$.

For $k \geq 2$, from the definition of $R[\iota, h, t, k]$ we have
$$R[\iota, h, t, k] = \sum_{u \in U} \iota(h)(u) \cdot R[\iota, hu, t, k - 1]~.$$
