...
Also remember that for all $h \in \mathcal{H}$ and $u \in U$ we have
$$|\sigma(h)(u) - \sigma_\epsilon(h)(u)| < c~.$$
Using this inequality, we prove by induction on $k$ that
\begin{align}
...
Now, we prove that the inequality \eqref{eq1:induction} holds for all $h \in \mathcal{H}$, $t \in T$, and $k \in \mathbb{N}$. The base case ($k = 1$) is trivial as $R[\iota_h, last(h), u, 1] = \iota(h)(u)$ and $|\sigma(h)(u) - \sigma_\epsilon(h)(u)| < c$.
